...
Partial Public Class WiaPropertiesForm : Inherits DevExpress.XtraEditors.XtraForm
    ...
    Private Sub WiaPropertiesForm_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
        _wiaProperties = WiaProperties.Empty
        ...
        Try
            ...
            '         _wiaProperties = MainForm._wiaSession.Properties
            'Else
            _wiaProperties = MainForm._wiaSession.GetProperties(MainForm._selectedWiaItem)
            'End If 
    ...
    Private Sub UpdateWiaPropertiesDlgControls()
        ...
        If ((_wiaProperties.ScanningMode And WiaScanningModeFlags.Flatbed) = WiaScanningModeFlags.Flatbed) OrElse (_wiaProperties.ScanningMode = WiaScanningModeFlags.None) Then
    ...
    Private Function GetWiaProperties() As Boolean
        ...
        If (_wiaProperties.ScanningMode And WiaScanningModeFlags.Feeder) = WiaScanningModeFlags.Feeder Then
            _cbFeeder.Checked = True
        End If
        If (_wiaProperties.ScanningMode And WiaScanningModeFlags.Flatbed) = WiaScanningModeFlags.Flatbed Then
            _cbFlatbed.Checked = True
        End If
        If (_wiaProperties.ScanningMode And WiaScanningModeFlags.Duplex) = WiaScanningModeFlags.Duplex Then
            _cbDuplex.Checked = True
        End If
        If (_wiaProperties.ScanningMode And WiaScanningModeFlags.FrontFirst) = WiaScanningModeFlags.FrontFirst Then
            _cbFrontFirst.Checked = True
        End If
        If (_wiaProperties.ScanningMode And WiaScanningModeFlags.BackFirst) = WiaScanningModeFlags.BackFirst Then
            _cbBackFirst.Checked = True
        End If
        If (_wiaProperties.ScanningMode And WiaScanningModeFlags.FrontOnly) = WiaScanningModeFlags.FrontOnly Then
            _cbFrontOnly.Checked = True
        End If
        If (_wiaProperties.ScanningMode And WiaScanningModeFlags.BackOnly) = WiaScanningModeFlags.BackOnly Then
            _cbBackOnly.Checked = True
        End If
        If (_wiaProperties.ScanningMode And WiaScanningModeFlags.NextPage) = WiaScanningModeFlags.NextPage Then
            _cbNextPage.Checked = True
        End If
        If (_wiaProperties.ScanningMode And WiaScanningModeFlags.Prefeed) = WiaScanningModeFlags.Prefeed Then
            _cbPrefeed.Checked = True
        End If
        If (_wiaProperties.ScanningMode And WiaScanningModeFlags.AutoAdvance) = WiaScanningModeFlags.AutoAdvance OrElse (_wiaProperties.ScanningMode And WiaScanningModeFlags.AdvancedDuplex) = WiaScanningModeFlags.AdvancedDuplex Then
            _cbAutoAdvance.Checked = True
        End If

        If _cmbOrientation.Items.Count > 0 Then
            HelperFunctions.SelectItemFromCombo(_cmbOrientation, CInt(_wiaProperties.Orientation))
        End If

        ' Check the Image Type check boxes according to the retrieved Image type flags.
        If _wiaProperties.ImageType = WiaImageType.Color Then
            _cbColored.Checked = True
            _cbGrayscale.Checked = False
            _cbText.Checked = False
        ElseIf _wiaProperties.ImageType = WiaImageType.Grayscale Then
            _cbGrayscale.Checked = True
            _cbColored.Checked = False
            _cbText.Checked = False
        ElseIf _wiaProperties.ImageType = WiaImageType.Text Then
        ...
        Else
            _numMaxPagesCount.Text = _wiaProperties.MaximumNumberOfPages.ToString()
        End If

        ' Select the Bits per pixel value from the combo box according to the retrieved one.
        If _cmbBitsPerPixel.Items.Count > 1 Then
            HelperFunctions.SelectItemFromCombo(_cmbBitsPerPixel, _wiaProperties.ImageResolution.BitsPerPixel)
        Else
            _cmbBitsPerPixel.Items.Clear()
            _cmbBitsPerPixel.Items.Add(_wiaProperties.ImageResolution.BitsPerPixel.ToString())
        ...
        If _cmbRotationAngle.Items.Count > 0 Then
            HelperFunctions.SelectItemFromCombo(_cmbRotationAngle, _wiaProperties.ImageResolution.RotationAngle)
        End If

        ' Set the Horizontal Resolution value according to the retrieved one.
        If _numHorizontalResolution.Visible = True Then
            _numHorizontalResolution.Value = _wiaProperties.ImageResolution.HorizontalResolution
        Else
            If _cmbHorizontalResolution.Items.Count > 0 Then
                HelperFunctions.SelectItemFromCombo(_cmbHorizontalResolution, _wiaProperties.ImageResolution.HorizontalResolution)
            End If
        End If

        ' Set the Vertical Resolution value according to the retrieved one.
        If _numVerticalResolution.Visible = True Then
            _numVerticalResolution.Value = _wiaProperties.ImageResolution.VerticalResolution
        Else
            If _cmbVerticalResolution.Items.Count > 0 Then
                HelperFunctions.SelectItemFromCombo(_cmbVerticalResolution, _wiaProperties.ImageResolution.VerticalResolution)
            End If
        End If

        ' Set the Horizontal Scaling value according to the retrieved one.
        _numHorizontalScaling.Value = _wiaProperties.ImageResolution.XScaling

        ' Set the Vertical Scaling value according to the retrieved one.
        _numVerticalScaling.Value = _wiaProperties.ImageResolution.YScaling

        ' Set the XPos value according to the retrieved one.
        _numXPos.Value = _wiaProperties.ImageResolution.XPos

        ' Set the YPos value according to the retrieved one.
        _numYPos.Value = _wiaProperties.ImageResolution.YPos

        ' Set the Width value according to the retrieved one.
        If (_wiaProperties.ImageResolution.Width > _numWidth.Maximum) Then
            _numWidth.Maximum = _wiaProperties.ImageResolution.Width
        End If
        _numWidth.Value = _wiaProperties.ImageResolution.Width

        ' Set the Height value according to the retrieved one.
        If (_wiaProperties.ImageResolution.Height > _numHeight.Maximum) Then
            _numHeight.Maximum = _wiaProperties.ImageResolution.Height
        End If
        _numHeight.Value = _wiaProperties.ImageResolution.Height

        ' Select the Transfer Format value from the combo box according to the retrieved one.
        If _cmbFormat.Items.Count > 0 Then
            '#If (Not LEADTOOLS_V17_OR_LATER) Then
            '         WiaSession.GetFormatGuid(_wiaProperties.DataTransfer.Format)
            '         Dim formatGuid As System.Guid = WiaSession.FormatGuid
            '#Else
            Dim formatGuid As System.Guid = WiaSession.GetFormatGuid(_wiaProperties.DataTransfer.Format)
            '#End If '#if !LEADTOOLS_V17_OR_LATER
        ...
        If _cmbCompression.Items.Count > 0 Then
            HelperFunctions.SelectItemFromCombo(_cmbCompression, CInt(_wiaProperties.DataTransfer.Compression))
        End If
        ...
        If _cmbImageDataType.Items.Count > 0 Then
            HelperFunctions.SelectItemFromCombo(_cmbImageDataType, CInt(_wiaProperties.DataTransfer.ImageDataType))
        End If
        ...
        _numBrightness.Value = _wiaProperties.ImageEffects.Brightness

        ' Set the Contrast value according to the retrieved one.
        _numContrast.Value = _wiaProperties.ImageEffects.Contrast
    ...
    Private Sub SetWiaProperties(ByVal item As Object)
        ...
        Dim dataTransfer As WiaDataTransferProperties = _wiaProperties.DataTransfer
        Dim imageResolution As WiaImageResolutionProperties = _wiaProperties.ImageResolution
        Dim imageEffects As WiaImageEffectsProperties = _wiaProperties.ImageEffects
        ...
        ElseIf _cbGrayscale.Checked Then
            _wiaProperties.ImageType = WiaImageType.Grayscale
        Else
            _wiaProperties.ImageType = WiaImageType.Text
        End If
        ...
        _wiaProperties.MaximumNumberOfPages = Convert.ToInt32(_numMaxPagesCount.Value)

        ' Set the user selected paper source and duplex mode.
        If _cbFeeder.Checked Then
            _wiaProperties.ScanningMode = _wiaProperties.ScanningMode Or WiaScanningModeFlags.Feeder
        ElseIf _cbFlatbed.Checked Then
            _wiaProperties.ScanningMode = _wiaProperties.ScanningMode Or WiaScanningModeFlags.Flatbed
        End If
        If _cbDuplex.Checked Then
            _wiaProperties.ScanningMode = _wiaProperties.ScanningMode Or WiaScanningModeFlags.Duplex
        End If
        If _cbAutoAdvance.Checked Then
            _wiaProperties.ScanningMode = _wiaProperties.ScanningMode Or WiaScanningModeFlags.AutoAdvance
        End If
        If _cbFrontFirst.Checked Then
            _wiaProperties.ScanningMode = _wiaProperties.ScanningMode Or WiaScanningModeFlags.FrontFirst
        End If
        If _cbBackFirst.Checked Then
            _wiaProperties.ScanningMode = _wiaProperties.ScanningMode Or WiaScanningModeFlags.BackFirst
        End If
        If _cbFrontOnly.Checked Then
            _wiaProperties.ScanningMode = _wiaProperties.ScanningMode Or WiaScanningModeFlags.FrontOnly
        End If
        If _cbBackOnly.Checked Then
            _wiaProperties.ScanningMode = _wiaProperties.ScanningMode Or WiaScanningModeFlags.BackOnly
        End If
        If _cbNextPage.Checked Then
            _wiaProperties.ScanningMode = _wiaProperties.ScanningMode Or WiaScanningModeFlags.NextPage
        End If
        If _cbPrefeed.Checked Then
            _wiaProperties.ScanningMode = _wiaProperties.ScanningMode Or WiaScanningModeFlags.Prefeed
        End If

        ' Set the user selected Orientation.
        If _cmbOrientation.Items.Count > 0 Then
            Dim myItem As MyItemData = CType(_cmbOrientation.SelectedItem, MyItemData)
            _wiaProperties.Orientation = CType(myItem.ItemData, WiaOrientation)
        End If
        ...
        _wiaProperties.DataTransfer = dataTransfer
        _wiaProperties.ImageResolution = imageResolution
        _wiaProperties.ImageEffects = imageEffects
        ...
        '      MainForm._wiaSession.Properties = _wiaProperties

        '      MainForm._wiaSession.SetProperties(item)
        '#Else
        ' Set the properties property first before calling the SetProperties function.
        MainForm._wiaSession.SetProperties(item, _wiaProperties)
        '#End If '#if !LEADTOOLS_V17_OR_LATER
    ...
    Private Sub FillFormatsListWithEnumeratedFormats()
        ...
        For Each i As MyFormat In MainForm._formatsList
            ...
            If i.TransferMode = _wiaProperties.DataTransfer.TransferMode Then
    ...
    Private Sub _cmbHorizontalResolution_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles _cmbHorizontalResolution.SelectedIndexChanged
        Try
            Dim ImageResProps As WiaImageResolutionProperties = _wiaProperties.ImageResolution
            Dim myItem As MyItemData = CType(_cmbHorizontalResolution.SelectedItem, MyItemData)
            ...
            _wiaProperties.ImageResolution = ImageResProps

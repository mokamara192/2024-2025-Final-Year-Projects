...
Public Class frmTransferPaymentDocuments
    ...
    Private Sub LoadRecords()
        Try
            ...
            Dim Query As String = " SELECT  ID,IPaymentID,FileType,FileName,GeneratedFileName,DocumentType,FileStatus,Description,RegDate,InputtedBy FROM TransferPaymentDocuments ORDER BY ID DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

            MyReportTable.Rows.Clear()
            ConTableAdapter.Fill(MyReportTable)

            TransferPaymentDocumentsDataGridView.DataSource = MyReportTable
    ...
    Private Sub cmdAdd_Click() 'Handles cmdAdd.Click
        ...
        If Not isCalled Then
            LoadedID = ""
            IPaymentIDGridBox.EditValue = ""
        End If
        IDTextBox.Text = ""
        FileTypeTextBox.Text = ""
        FileNameTextBox.Text = ""
        GeneratedFileNameTextBox.Text = ""
    ...
    Private Sub cmdSave_Click() 'Handles cmdSave.Click
        Try
            ...
            Dim Query As String = "INSERT INTO TransferPaymentDocuments (IPaymentID,FileType,FileName,GeneratedFileName,DocumentType,FileStatus,Description,RegDate,InputtedBy) VALUES(@IPaymentID,@FileType,@FileName,@GeneratedFileName,@DocumentType,@FileStatus,@Description,@RegDate,@InputtedBy)"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@ID", IDTextBox.Text)
                .AddWithValue("@IPaymentID", IPaymentIDGridBox.EditValue)
                .AddWithValue("@FileType", FileTypeTextBox.Text)
                .AddWithValue("@FileName", FileNameTextBox.Text)
                .AddWithValue("@GeneratedFileName", GeneratedFileNameTextBox.Text)
                .AddWithValue("@DocumentType", DocumentTypeTextBox.Text)
                .AddWithValue("@FileStatus", FileStatusTextBox.Text)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "TransferPaymentDocuments" & " With ID (" & IDTextBox.Text & ")", 0)
            LoadRecords()
            'cmdAdd_Click()
            DisplayRecordInfo(IDTextBox.Text)

        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that IPaymentID:TransferPayments:IPaymentID have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
    ...
    Private Sub cmdUpdate_Click() 'Handles cmdUpdate.Click
        Try
            ...
            Dim Query As String = "UPDATE TransferPaymentDocuments SET IPaymentID =@IPaymentID, FileType =@FileType, FileName =@FileName, GeneratedFileName =@GeneratedFileName, DocumentType =@DocumentType, FileStatus =@FileStatus, Description =@Description, RegDate =@RegDate, InputtedBy =@InputtedBy WHERE ID = '" & LoadedID & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@ID", IDTextBox.Text)
                .AddWithValue("@IPaymentID", IPaymentIDGridBox.EditValue)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "TransferPaymentDocuments" & " With ID (" & IDTextBox.Text & ")", 0)
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that IPaymentID:TransferPayments:IPaymentID have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
        ...
        If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                If PanelNew.Visible Then
                    ThePKey = IDTextBox.Text
                Else
                    ThePKey = GridView1.GetFocusedDataRow.Item("ID")
                End If

                Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                Dim Query As String = "DELETE FROM TransferPaymentDocuments WHERE ID = '" & ThePKey & "'"
                Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                ...
                CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "TransferPaymentDocuments" & " With ID (" & ThePKey & ")", 0)
                LoadRecords()
                Dim TheNewPKey As String = ""
                If MyReportTable.Rows.Count > 0 Then
                    TheNewPKey = GridView1.GetFocusedDataRow.Item("ID")
                End If
    ...
    'Private Sub IDTextBox_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles IDTextBox.TextChanged
    'DisplayRecordInfo(IDTextBox.Text)
    'End Sub

    Private Sub LoadIPaymentID()

        Try
            Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
            Dim Query As String = "Select  IPaymentID,GeneratedTransferNo,PaidBy From TransferPayments"

            Dim ConTableAdap As New SqlClient.SqlDataAdapter(Query, Conn)
            Dim MyTableTransferPayments As New DataTable

            ConTableAdap.Fill(MyTableTransferPayments)

            IPaymentIDGridBox.Properties.DataSource = Nothing
            IPaymentIDGridBox.Properties.DataSource = MyTableTransferPayments

            IPaymentIDGridBox.Properties.DisplayMember = "GeneratedTransferNo"
            IPaymentIDGridBox.Properties.ValueMember = "IPaymentID"
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Public Sub DisplayRecordInfo(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = " SELECT ID,IPaymentID,FileType,FileName,GeneratedFileName,DocumentType,FileStatus,Description,RegDate,InputtedBy FROM TransferPaymentDocuments WHERE ID = '" & RecordID & "'"
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
            ...
            If MySingleReportTable.Rows.Count > 0 Then
                ...
                IDTextBox.Text = MySingleReportTable.Rows(0).Item("ID").ToString
                IPaymentIDGridBox.EditValue = MySingleReportTable.Rows(0).Item("IPaymentID").ToString
                FileTypeTextBox.Text = MySingleReportTable.Rows(0).Item("FileType").ToString
                FileNameTextBox.Text = MySingleReportTable.Rows(0).Item("FileName").ToString
                GeneratedFileNameTextBox.Text = MySingleReportTable.Rows(0).Item("GeneratedFileName").ToString
                DocumentTypeTextBox.Text = MySingleReportTable.Rows(0).Item("DocumentType").ToString
                FileStatusTextBox.Text = MySingleReportTable.Rows(0).Item("FileStatus").ToString
    ...
    '       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportTransferPaymentDocuments
    '       MyDocument.SummaryInfo.ReportTitle = "Transfer Payment Documents"
    '	      Dim MyTable As New DataTable
    '       Dim TableName As String = "TransferPaymentDocuments"
    '       Dim Fields As String = "ID"
    '       Dim Values As String = IDTextBox.Text
    ...
    '           Dim QrCodeData As String = "TransferPaymentDocuments Report - Printed on: " & DateTime.Today
    ...
    Private Sub SearchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SearchToolStripMenuItem.Click
        ...
        Try
            ...
            MySearch.FormName = "frmTransferPaymentDocuments"
            MySearch.TableName = "TransferPaymentDocuments"
            MySearch.PrimaryKey = "ID"
    ...
    Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("TransferPaymentDocuments")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("TransferPaymentDocuments")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub OpenToolStripButton_Click() Handles OpenToolStripButton.Click
        Try
            Dim DRecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("ID"))
    ...
    Private Sub SearchToolStripButton_Click() Handles SearchToolStripButton.Click
        ...
        Try
            Dim QueryItems As String = "ID,IPaymentID,FileType,FileName,GeneratedFileName,DocumentType,FileStatus,Description,RegDate,InputtedBy"
            Dim TableName As String = "TransferPaymentDocuments"
            ...
            If SearchToolStripComboBox.Text = "" Then
                Condition = ""
            Else
                Condition = "ID = '" & SearchToolStripComboBox.Text & "'"
            End If

            Dim OrderByCondition As String = "ORDER BY ID DESC"
            ...
            Else
                OpenNEWToolStripButton_Click()
                DisplayRecordInfo(MyTable.Rows(0).Item("ID"))
            End If
            isDateSearchUsed = False
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "TransferPaymentDocuments", "ID = '" & DRecordID & "'", " ORDER BY ID ASC")
...
#Region "EXPORTING AND OTHER FUNCTIONS FROM DASHBOARD BUTTONS"
    ...
    Private Sub frmTransferPaymentDocuments_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
#End Region
    ...
    Private Sub PrintToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintToolStripMenuItem.Click
        Try
            ...
            If PanelNew.Visible = False Then
                Dim RecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("ID"))
            ...
            Dim MyDocument As New XtraReportTransferPaymentDocumentsSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.TransferPaymentDocumentsDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "Transfer Payment Documents"
    ...
    Private Sub PrintAllListedRecordsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintAllListedRecordsToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportTransferPaymentDocumentsMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.TransferPaymentDocumentsDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "TransferPaymentDocuments"
...
#Region "SEARCH BY DATE"
    ...
    Private Sub FormLoadingSetup() Handles Me.Load
        Try
            ...
            Dim Allfields As String = GetAllFields("TransferPaymentDocuments", DatabaseName)
            ...
            For Each field In splitfields
                ...
                Dim ColumnType As String = GetColumnDataType("TransferPaymentDocuments", field).ToUpper
    ...
    Private Sub cmdDateSearch_Click(sender As Object, e As EventArgs) Handles cmdDateSearch.Click
        If SearchDateFieldComboBox.SelectedIndex <> -1 Then
            Try
                ...
                Dim Query As String = " SELECT ID,IPaymentID,FileType,FileName,GeneratedFileName,DocumentType,FileStatus,Description,RegDate,InputtedBy FROM TransferPaymentDocuments WHERE " & SearchDateFieldComboBox.Text & " BETWEEN '" & StartDateTimePicker.DateTime.ToString("yyy-MM-dd") & "' AND '" & EndDateTimePicker.DateTime.ToString("yyyy-MM-dd") & "' ORDER BY ID DESC "
                Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
...
#Region "DOCUMENT FILE MANAGEMENT"
    ...
    Private Sub cmdAddFile_Click(sender As Object, e As EventArgs) Handles cmdAddFile.Click
        PanelSelectaDocumentSource.Visible = True
        PanelSelectaDocumentSource.BringToFront()

    End Sub


    Private Sub cmdScanDocument_Click(sender As Object, e As EventArgs) Handles cmdScanDocument.Click
        Try
            Dim MyForm As New frmScanner

            If MyForm.ShowDialog = System.Windows.Forms.DialogResult.OK Then

                Dim therandom As New Random

                Dim thevalue As String = therandom.NextDouble().ToString().Substring(5, 3)

                Dim FileName As String = "SCN_ACC" & DateTime.Now.ToString("yyyyMMdd-HHmmss") & thevalue & ".jpg"

                Dim MyMemoryStream As New System.IO.MemoryStream()
                MyForm.ScanPictureBox.Image.Save(MyMemoryStream, System.Drawing.Imaging.ImageFormat.Jpeg)

                SaveToolStripButton.PerformClick()

                GeneratedFileNameTextBox.Text = FileName
                InsertANewDocument(FileName, MyMemoryStream.ToArray())
    ...
    Private Sub InsertANewDocument(ByVal FileName As String, ByVal Optional FileData As Byte() = Nothing)
        Try
            Dim DExtension As String() = FileName.Trim.Split(".")
            Dim FileExtension As String = DExtension(DExtension.Count - 1)

            Dim GenerateFileName As String = GenerateDocumentFileName(FileExtension, "ID", "TransferPaymentDocuments")
            ...
            If FileName.Contains("\") Then
                ...
                If FileName.ToUpper.EndsWith("JPG") Or FileName.ToUpper.EndsWith("JPEG") Or FileName.ToUpper.EndsWith("PNG") Or FileName.ToUpper.EndsWith("BMP") Then
                    ...
                    InsertDocumentToFileStream(GenerateFileName, MyMemoryStream.ToArray, "TransferPaymentDocuments")
                Else
                    Dim TheBytes As Byte() = My.Computer.FileSystem.ReadAllBytes(FileName)
                    InsertDocumentToFileStream(GenerateFileName, TheBytes, "TransferPaymentDocuments")
                End If

                GeneratedFileNameTextBox.Text = GenerateFileName

                cmdRemoveFile.Enabled = True
                cmdAddFile.Enabled = False
                cmdViewFile.Enabled = True
                cmdDownloadFile.Enabled = True
                cmdViewAllFiles.Enabled = True
            Else
                GeneratedFileNameTextBox.Text = GenerateFileName
                InsertDocumentToFileStream(GenerateFileName, FileData, "TransferPaymentDocuments")

                cmdRemoveFile.Enabled = True
                cmdAddFile.Enabled = False
                cmdViewFile.Enabled = False
                cmdDownloadFile.Enabled = False
                cmdViewAllFiles.Enabled = False
            End If

            CreateLog(CurrentUserID, FullNameofUser, "Insert of new Transfer Payment Documents record for : " & IPaymentIDGridBox.EditValue, 0)
    ...
    Private Sub cmdRemoveData_Click(sender As Object, e As EventArgs) Handles cmdRemoveFile.Click

        If MessageBox.Show("Do you really want to delete the file?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                Dim DocumentID As String = IDTextBox.Text

                'DELETING THE RECORD FROM THE FILE SERVER
                Dim DocumentGeneratedName As String = GetDocumentGeneratedName(DocumentID, "GeneratedFileName", "TransferPaymentDocuments", "ID")
                DeleteDocumentFromFileStream(DocumentGeneratedName, "TransferPaymentDocuments")

                CreateLog(CurrentUserID, FullNameofUser, "Delete of Transfer Payment Documents File for :" & IPaymentIDGridBox.EditValue, 0)
    ...
    Private Sub cmdBrowseForFile_Click(sender As Object, e As EventArgs) Handles cmdBrowseForFile.Click
        Try
            ...
            'If FileTypeTextBox.Text = "" Then
            'MessageBox.Show("Please specify the File Type before proceeding.", "Missing Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            'FileTypeTextBox.Focus()
            ...
            If OFD1.ShowDialog = System.Windows.Forms.DialogResult.OK Then
                FileNameTextBox.Text = OFD1.SafeFileName
                'Generated filename is being acquired from the InsertANewDocument Method

                InsertANewDocument(OFD1.FileName)
                SaveToolStripButton.PerformClick()
                Me.PanelSelectaDocumentSource.Visible = False

                MessageBox.Show("File successfully uploaded to the Server.", "Saved", MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If
    ...
    Private Function ReduceImageSize(ByVal ImageSource As String, ByVal widthpercent As Single, ByVal heightpercent As Single) As Bitmap
        Try
    ...
    Private Sub cmdViewFile_Click(sender As Object, e As EventArgs) Handles cmdViewFile.Click
        Dim ItemDescription As String = DescriptionTextBox.Text
        Dim TheSplit As String() = ItemDescription.Trim.Split(":")
        Dim TheFileName As String = FileNameTextBox.Text
        ...
        If TheFileName.ToUpper.Contains(".JPG") Or TheFileName.ToUpper.Contains(".PNG") Or TheFileName.ToUpper.Contains(".BMP") Or TheFileName.ToUpper.Contains(".JPEG") Then
            Dim TheBytes As Byte() = GetDocumentData(IDTextBox.Text, "GeneratedFileName", "TransferPaymentDocuments", "ID")
        ...
        ElseIf TheFileName.ToUpper.Contains(".MP4") Or TheFileName.ToUpper.Contains(".AVI") Or TheFileName.ToUpper.Contains(".MPEG") Or TheFileName.ToUpper.Contains(".3GP") Or TheFileName.ToUpper.Contains(".MKV") Or TheFileName.ToUpper.Contains(".MP3") Or TheFileName.ToUpper.Contains(".WAV") Or TheFileName.ToUpper.Contains(".M4A") Then
            ...
            MyForm.FileNameTextBox.Text = FileNameTextBox.Text
            MyForm.RegDateTextBox.Text = "Date: " & RegDateTextBox.Text
            MyForm.PreparePlaylist()

            Dim FullPath As String = "\\" & FILEServerIPAddress & "\" & FILEFolder & "\TransferPaymentDocuments\" & GeneratedFileNameTextBox.Text
    ...
    Private Sub cmdDownloadFile_Click(sender As Object, e As EventArgs) Handles cmdDownloadFile.Click
        Try
            Dim TheBytes As Byte() = GetDocumentData(IDTextBox.Text, "GeneratedFileName", "TransferPaymentDocuments", "ID")
            Dim ItemDescription As String = DescriptionTextBox.Text
            Dim TheSplit As String() = ItemDescription.Trim.Split(":")
    ...
    Private Sub cmdViewAllFiles_Click(sender As Object, e As EventArgs) Handles cmdViewAllFiles.Click

        Dim MyForm As New frmOpenandViewDocuments
        MyForm.ForeignKeyField = "IPaymentID"
        MyForm.ForeignKeyValue = IPaymentIDGridBox.EditValue
        MyForm.TableName = "TransferPaymentDocuments"

        MyForm.BarStaticItemTransactionNo.Caption = IPaymentIDGridBox.EditValue

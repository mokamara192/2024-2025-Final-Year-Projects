...
Public Class frmTransferFilesData
    ...
    Private Sub LoadRecords()
        Try
            ...
            Dim Query As String = " SELECT  ID,GeneratedTransferNo,FileType,FileName,GeneratedFileName,FileStatus,Description,RegDate,InputtedBy FROM TransferFilesData ORDER BY ID DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

            MyReportTable.Rows.Clear()
            ConTableAdapter.Fill(MyReportTable)

            TransferFilesDataDataGridView.DataSource = MyReportTable
    ...
    Private Sub cmdSave_Click() 'Handles cmdSave.Click
        Try
            ...
            Dim Query As String = "INSERT INTO TransferFilesData (GeneratedTransferNo,FileType,FileName,GeneratedFileName,FileStatus,Description,RegDate,InputtedBy) VALUES(@GeneratedTransferNo,@FileType,@FileName,@GeneratedFileName,@FileStatus,@Description,@RegDate,@InputtedBy)"
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "TransferFilesData" & " With ID (" & IDTextBox.Text & ")", 0)
            LoadRecords()
            'cmdAdd_Click()
    ...
    Private Sub cmdUpdate_Click() 'Handles cmdUpdate.Click
        Try
            ...
            Dim Query As String = "UPDATE TransferFilesData SET GeneratedTransferNo =@GeneratedTransferNo, FileType =@FileType, FileName =@FileName, GeneratedFileName =@GeneratedFileName, FileStatus =@FileStatus, Description =@Description, RegDate =@RegDate, InputtedBy =@InputtedBy WHERE ID = '" & LoadedID & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@ID", IDTextBox.Text)
                .AddWithValue("@GeneratedTransferNo", GeneratedTransferNoGridBox.EditValue)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "TransferFilesData" & " With ID (" & IDTextBox.Text & ")", 0)
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that GeneratedTransferNo:Transfers:GeneratedTransferNo have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
    ...
    Private Sub cmdDelete_Click() 'Handles cmdDelete.Click
        If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                Dim Query As String = "DELETE FROM TransferFilesData WHERE ID = '" & ThePKey & "'"
                Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                ...
                CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "TransferFilesData" & " With ID (" & ThePKey & ")", 0)
                LoadRecords()
    ...
    Public Sub DisplayRecordInfo(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = " SELECT ID,GeneratedTransferNo,FileType,FileName,GeneratedFileName,FileStatus,Description,RegDate,InputtedBy FROM TransferFilesData WHERE ID = '" & RecordID & "'"
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
    ...
    '       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportTransferFilesData
    '       MyDocument.SummaryInfo.ReportTitle = "Transfer Files Data"
    '	      Dim MyTable As New DataTable
    '       Dim TableName As String = "TransferFilesData"
    ...
    '           Dim QrCodeData As String = "TransferFilesData Report - Printed on: " & DateTime.Today
    ...
    Private Sub SearchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SearchToolStripMenuItem.Click
        ...
        Try
            ...
            MySearch.FormName = "frmTransferFilesData"
            MySearch.TableName = "TransferFilesData"
    ...
    Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("TransferFilesData")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("TransferFilesData")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub SearchToolStripButton_Click() Handles SearchToolStripButton.Click
        ...
        Try
            Dim QueryItems As String = "ID,GeneratedTransferNo,FileType,FileName,GeneratedFileName,FileStatus,Description,RegDate,InputtedBy"
            Dim TableName As String = "TransferFilesData"
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "TransferFilesData", "ID = '" & DRecordID & "'", " ORDER BY ID ASC")
...
#Region "EXPORTING AND OTHER FUNCTIONS FROM DASHBOARD BUTTONS"
    ...
    Private Sub frmTransferFilesData_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
#End Region
    ...
    Private Sub PrintToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportTransferFilesDataSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.TransferFilesDataDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "Transfer Files Data"
    ...
    Private Sub PrintAllListedRecordsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintAllListedRecordsToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportTransferFilesDataMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.TransferFilesDataDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "TransferFilesData"
...
#Region "SEARCH BY DATE"
    ...
    Private Sub FormLoadingSetup() Handles Me.Load
        Try
            ...
            Dim Allfields As String = GetAllFields("TransferFilesData", DatabaseName)
            ...
            For Each field In splitfields
                ...
                Dim ColumnType As String = GetColumnDataType("TransferFilesData", field).ToUpper
    ...
    Private Sub cmdDateSearch_Click(sender As Object, e As EventArgs) Handles cmdDateSearch.Click
        If SearchDateFieldComboBox.SelectedIndex <> -1 Then
            Try
                ...
                Dim Query As String = " SELECT ID,GeneratedTransferNo,FileType,FileName,GeneratedFileName,FileStatus,Description,RegDate,InputtedBy FROM TransferFilesData WHERE " & SearchDateFieldComboBox.Text & " BETWEEN '" & StartDateTimePicker.DateTime.ToString("yyy-MM-dd") & "' AND '" & EndDateTimePicker.DateTime.ToString("yyyy-MM-dd") & "' ORDER BY ID DESC "
                Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
...
#Region "DOCUMENT FILE MANAGEMENT"
    ...
    Private Sub InsertANewDocument(ByVal FileName As String, ByVal Optional FileData As Byte() = Nothing)
        Try
            ...
            Dim GenerateFileName As String = GenerateDocumentFileName(FileExtension, "ID", "TransferFilesData")
            ...
            If FileName.Contains("\") Then
                ...
                If FileName.ToUpper.EndsWith("JPG") Or FileName.ToUpper.EndsWith("JPEG") Or FileName.ToUpper.EndsWith("PNG") Or FileName.ToUpper.EndsWith("BMP") Then
                    ...
                    InsertDocumentToFileStream(GenerateFileName, MyMemoryStream.ToArray, "TransferFilesData")
                Else
                    Dim TheBytes As Byte() = My.Computer.FileSystem.ReadAllBytes(FileName)
                    InsertDocumentToFileStream(GenerateFileName, TheBytes, "TransferFilesData")
                End If
            ...
            Else
                GeneratedFileNameTextBox.Text = GenerateFileName
                InsertDocumentToFileStream(GenerateFileName, FileData, "TransferFilesData")
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new Transfer Files Data record for : " & GeneratedTransferNoGridBox.EditValue, 0)
    ...
    Private Sub cmdRemoveData_Click(sender As Object, e As EventArgs) Handles cmdRemoveFile.Click
        ...
        If MessageBox.Show("Do you really want to delete the file?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                Dim DocumentGeneratedName As String = GetDocumentGeneratedName(DocumentID, "GeneratedFileName", "TransferFilesData", "ID")
                DeleteDocumentFromFileStream(DocumentGeneratedName, "TransferFilesData")

                CreateLog(CurrentUserID, FullNameofUser, "Delete of Transfer Files Data File for :" & GeneratedTransferNoGridBox.EditValue, 0)
    ...
    Private Sub cmdViewFile_Click(sender As Object, e As EventArgs) Handles cmdViewFile.Click
        ...
        If TheFileName.ToUpper.Contains(".JPG") Or TheFileName.ToUpper.Contains(".PNG") Or TheFileName.ToUpper.Contains(".BMP") Or TheFileName.ToUpper.Contains(".JPEG") Then
            Dim TheBytes As Byte() = GetDocumentData(IDTextBox.Text, "GeneratedFileName", "TransferFilesData", "ID")
        ...
        ElseIf TheFileName.ToUpper.Contains(".MP4") Or TheFileName.ToUpper.Contains(".AVI") Or TheFileName.ToUpper.Contains(".MPEG") Or TheFileName.ToUpper.Contains(".3GP") Or TheFileName.ToUpper.Contains(".MKV") Or TheFileName.ToUpper.Contains(".MP3") Or TheFileName.ToUpper.Contains(".WAV") Or TheFileName.ToUpper.Contains(".M4A") Then
            ...
            Dim FullPath As String = "\\" & FILEServerIPAddress & "\" & FILEFolder & "\TransferFilesData\" & GeneratedFileNameTextBox.Text
    ...
    Private Sub cmdViewAllFiles_Click(sender As Object, e As EventArgs) Handles cmdViewAllFiles.Click
        ...
        MyForm.ForeignKeyValue = GeneratedTransferNoGridBox.EditValue
        MyForm.TableName = "TransferFilesData"

        MyForm.BarStaticItemTransactionNo.Caption = GeneratedTransferNoGridBox.EditValue

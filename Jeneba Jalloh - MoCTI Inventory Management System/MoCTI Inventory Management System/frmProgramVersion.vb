...
Public Class frmProgramVersion
    ...
    Dim LoadedProgramVersion As String
...
Dim Query as String = " SELECT  ProgramVersion,ExpiryDate,ProgramStatus,ComputerName FROM ProgramVersion ORDER BY ProgramVersion DESC "
Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

MyReportTable.Rows.Clear
ConTableAdapter.Fill(MyReportTable)

ProgramVersionDataGridView.Datasource = MyReportTable
isDateSearchUsed = False
'If MyReportTable.Rows.Count > 0 Then
'DisplayRecordInfo(MyReportTable.Rows(0).Item("ProgramVersion"))
'End If
...
Private Function ValidateID(ByVal ProgramVersion As String) As Boolean
Try
Dim Conn as New Sqlclient.SqlConnection(MyConnectionString)

Dim Query as String = "SELECT Count(*) FROM ProgramVersion WHERE ProgramVersion = '" & ProgramVersion & "'"
...
Dim Query as String = "SELECT Count(*) FROM ProgramVersion WHERE ProgramVersion LIKE 'PRO" & DateTime.Today.ToString("yyMMdd") & "%'"
Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
...
Recordcount += 1
                If Recordcount >= 0 And Recordcount < 10 Then
                    Start = "PRO" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "000" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 10 And Recordcount < 100 Then
                    Start = "PRO" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "00" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 100 And Recordcount < 1000 Then
                    Start = "PRO" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "0" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 1000 And Recordcount < 10000 Then
                    Start = "PRO" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
...
LoadedProgramVersion = ""
 End If
ProgramVersionTextBox.Text =  ""
ExpiryDateDateTimePicker.DateTime = DateTime.Now
ProgramStatusTextBox.Text =  ""
ComputerNameTextBox.Text =  ""

ProgramVersionTextBox.Text = GenerateID

End Sub

Private Sub cmdSave_Click() 'Handles cmdSave.Click
Try
            If ProgramVersionTextBox.Text <> "" Then
                If ValidateID(ProgramVersionTextBox.Text) Then
                    MessageBox.Show("This Program Version already exist in this table. DUPLICATE 'PROGRAM VERSION' ARE NOT ALLOWED. Please verify and try again.", "Already Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
...
Dim Query as String = "INSERT INTO ProgramVersion (ProgramVersion,ExpiryDate,ProgramStatus,ComputerName) VALUES(@ProgramVersion,@ExpiryDate,@ProgramStatus,@ComputerName)"
Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
With ConCommand.Parameters
.AddWithValue("@ProgramVersion", ProgramVersionTextBox.Text)
.AddWithValue("@ExpiryDate", ExpiryDateDateTimePicker.DateTime.Date)
.AddWithValue("@ProgramStatus", ProgramStatusTextBox.Text)
.AddWithValue("@ComputerName", ComputerNameTextBox.Text)
End With
...
CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "ProgramVersion" & " With ID (" & ProgramVersionTextBox.Text & ")", 0)
LoadRecords()
'cmdAdd_Click()
DisplayRecordInfo(ProgramVersionTextBox.Text)
...
Try
                If Not ValidateID(LoadedProgramVersion) Then
                    MessageBox.Show("The loaded Program Version (" & LoadedProgramVersion & ") was not found in the database table. UPDATE OPERATION WILL NOT BE SUCCESSFULL. Please verify and try again.", "Not Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
...
Dim Query as String = "UPDATE ProgramVersion SET ExpiryDate =@ExpiryDate, ProgramStatus =@ProgramStatus, ComputerName =@ComputerName WHERE ProgramVersion = '" & LoadedProgramVersion & "'"
Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
With ConCommand.Parameters
.AddWithValue("@ProgramVersion", ProgramVersionTextBox.Text)
.AddWithValue("@ExpiryDate", ExpiryDateDateTimePicker.DateTime.Date)
...
CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "ProgramVersion" & " With ID (" & ProgramVersionTextBox.Text & ")", 0)
Catch Ex as Exception
           If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that  have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message , "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
           Else
...
Try
 ...
 ThePKey = ProgramVersionTextBox.Text
 Else
 ThePKey = GridView1.GetFocusedDataRow.Item("ProgramVersion")
 End If
 
Dim Conn as New Sqlclient.SqlConnection(MyConnectionString)
Dim Query as String = "DELETE FROM ProgramVersion WHERE ProgramVersion = '" & ThePKey & "'"
Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
...
CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "ProgramVersion" & " With ID (" & ThePKey & ")", 0)
LoadRecords()
 Dim TheNewPKey As String = ""
 If MyReportTable.Rows.Count > 0 Then
 TheNewPKey = GridView1.GetFocusedDataRow.Item("ProgramVersion")
 End If
...
'Private Sub ProgramVersionTextBox_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles ProgramVersionTextBox.TextChanged
'DisplayRecordInfo(ProgramVersionTextBox.Text)
'End Sub
...
Dim Query as String = " SELECT ProgramVersion,ExpiryDate,ProgramStatus,ComputerName FROM ProgramVersion WHERE ProgramVersion = '"& RecordID & "'"
Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
...
ProgramVersionTextBox.Text = MySingleReportTable.Rows(0).Item("ProgramVersion").ToString
ExpiryDateDateTimePicker.DateTime = MySingleReportTable.Rows(0).Item("ExpiryDate")
ProgramStatusTextBox.Text = MySingleReportTable.Rows(0).Item("ProgramStatus").ToString
ComputerNameTextBox.Text = MySingleReportTable.Rows(0).Item("ComputerName").ToString
LoadedProgramVersion = ProgramVersionTextBox.Text
...
'       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportProgramVersion
'       MyDocument.SummaryInfo.ReportTitle = "Program Version"
'	      Dim MyTable As New DataTable
'       Dim TableName As String = "ProgramVersion"
'       Dim Fields As String = "ProgramVersion"
'       Dim Values As String = ProgramVersionTextBox.Text
...
'           Dim QrCodeData As String = "ProgramVersion Report - Printed on: " & DateTime.Today
...
End Sub
	...
	Try
            ...
            MySearch.FormName = "frmProgramVersion"
            MySearch.TableName = "ProgramVersion"
            MySearch.PrimaryKey = "ProgramVersion"
 ...
 Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("ProgramVersion")
        Catch ex As Exception
            MessageBox.Show(Ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
   ...
   Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("ProgramVersion")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub OpenToolStripButton_Click() Handles OpenToolStripButton.Click
      Try
	    Dim DRecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("ProgramVersion"))
    ...
    Private Sub SearchToolStripButton_Click() Handles SearchToolStripButton.Click
        ...
        Try
            Dim QueryItems As String = "ProgramVersion,ExpiryDate,ProgramStatus,ComputerName"
            Dim TableName As String = "ProgramVersion"
            ...
            If SearchToolStripComboBox.Text = "" Then
                Condition = ""
            Else
                Condition = "ProgramVersion = '" & SearchToolStripComboBox.Text & "'"
            End If

            Dim OrderByCondition As String = "ORDER BY ProgramVersion DESC"
	    ...
	    Dim MyTable As DataTable = GetDataForTable(QueryItems, TableName, Condition, OrderByCondition)
            ...
            Else
                OpenNEWToolStripButton_Click()
                DisplayRecordInfo(MyTable.Rows(0).Item("ProgramVersion"))
            End If
            isDateSearchUsed = False
        Catch ex As Exception
            MessageBox.Show(Ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub MySearchItemsLoading() Handles Me.Load
        Dim QueryItem As String = "ProgramVersion"
        Dim TableName As String = "ProgramVersion"
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
      ...
      'DisplayRecordInfo(DRecordID)
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "ProgramVersion", "ProgramVersion = '" & DRecordID & "'", " ORDER BY ProgramVersion ASC")
...
Private Sub frmProgramVersion_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
Dim RecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("ProgramVersion"))
...
Exit Sub
 End If
            ...
            Dim MyDocument As New XtraReportProgramVersionSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.ProgramVersionDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "Program Version"
...
Exit Sub
 End If
            ...
            Dim MyDocument As New XtraReportProgramVersionMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.ProgramVersionDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "ProgramVersion"
...
#Region "SEARCH BY DATE"
    ...
    Private Sub FormLoadingSetup() Handles Me.Load
        Try
            ...
            Dim Allfields As String = GetAllFields("ProgramVersion", DatabaseName)
            ...
            For Each field In splitfields
                ...
                Dim ColumnType As String = GetColumnDataType("ProgramVersion", field).ToUpper
    ...
    Private Sub cmdDateSearch_Click(sender As Object, e As EventArgs) Handles cmdDateSearch.Click
        If SearchDateFieldComboBox.SelectedIndex <> -1 Then
            Try
                ...
                Dim Query As String = " SELECT ProgramVersion,ExpiryDate,ProgramStatus,ComputerName FROM ProgramVersion WHERE " & SearchDateFieldComboBox.Text & " BETWEEN '" & StartDateTimePicker.DateTime.ToString("yyy-MM-dd") & "' AND '" & EndDateTimePicker.DateTime.ToString("yyyy-MM-dd") & "' ORDER BY ProgramVersion DESC "
                Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

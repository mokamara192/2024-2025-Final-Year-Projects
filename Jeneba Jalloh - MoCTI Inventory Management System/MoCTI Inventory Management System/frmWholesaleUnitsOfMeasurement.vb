...
Public Class frmWholesaleUnitsOfMeasurement
    ...
    Dim LoadedWholesaleUnitOfMeasurement As String
    ...
    Private Sub LoadRecords()
        Try
            ...
            Dim Query As String = " SELECT  WholesaleUnitOfMeasurement FROM WholesaleUnitsOfMeasurement ORDER BY WholesaleUnitOfMeasurement DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

            MyReportTable.Rows.Clear()
            ConTableAdapter.Fill(MyReportTable)

            WholesaleUnitsOfMeasurementDataGridView.DataSource = MyReportTable
            isDateSearchUsed = False
            'If MyReportTable.Rows.Count > 0 Then
            'DisplayRecordInfo(MyReportTable.Rows(0).Item("WholesaleUnitOfMeasurement"))
            'End If
    ...
    Private Function ValidateID(ByVal WholesaleUnitOfMeasurement As String) As Boolean
        Try
            Dim Conn As New SqlClient.SqlConnection(MyConnectionString)

            Dim Query As String = "SELECT Count(*) FROM WholesaleUnitsOfMeasurement WHERE WholesaleUnitOfMeasurement = '" & WholesaleUnitOfMeasurement & "'"
    ...
    Private Function GenerateID() As String
        Try
            ...
            Dim Query As String = "SELECT Count(*) FROM WholesaleUnitsOfMeasurement WHERE WholesaleUnitOfMeasurement LIKE 'WHO" & DateTime.Today.ToString("yyMMdd") & "%'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            ...
            While IsIDGenerated = False
                ...
                If Recordcount >= 0 And Recordcount < 10 Then
                    Start = "WHO" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "000" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 10 And Recordcount < 100 Then
                    Start = "WHO" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "00" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 100 And Recordcount < 1000 Then
                    Start = "WHO" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "0" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 1000 And Recordcount < 10000 Then
                    Start = "WHO" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
    ...
    Private Sub cmdAdd_Click() 'Handles cmdAdd.Click
        ...
        If Not isCalled Then
            LoadedWholesaleUnitOfMeasurement = ""
        End If
        WholesaleUnitOfMeasurementTextBox.Text = ""

        'WholesaleUnitOfMeasurementTextBox.Text = "Auto"

    End Sub

    Private Sub cmdSave_Click() 'Handles cmdSave.Click
        Try
            If WholesaleUnitOfMeasurementTextBox.Text <> "" Then
                If ValidateID(WholesaleUnitOfMeasurementTextBox.Text) Then
                    MessageBox.Show("This Wholesale Unit Of Measurement already exist in this table. DUPLICATE 'WHOLESALE UNIT OF MEASUREMENT' ARE NOT ALLOWED. Please verify and try again.", "Already Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "INSERT INTO WholesaleUnitsOfMeasurement (WholesaleUnitOfMeasurement) VALUES(@WholesaleUnitOfMeasurement)"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@WholesaleUnitOfMeasurement", WholesaleUnitOfMeasurementTextBox.Text)
            End With
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "WholesaleUnitsOfMeasurement" & " With ID (" & WholesaleUnitOfMeasurementTextBox.Text & ")", 0)
            LoadRecords()
            'cmdAdd_Click()
            DisplayRecordInfo(WholesaleUnitOfMeasurementTextBox.Text)
    ...
    Private Sub cmdUpdate_Click() 'Handles cmdUpdate.Click
        Try
            If Not ValidateID(LoadedWholesaleUnitOfMeasurement) Then
                MessageBox.Show("The loaded Wholesale Unit Of Measurement (" & LoadedWholesaleUnitOfMeasurement & ") was not found in the database table. UPDATE OPERATION WILL NOT BE SUCCESSFULL. Please verify and try again.", "Not Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "UPDATE WholesaleUnitsOfMeasurement SET  WHERE WholesaleUnitOfMeasurement = '" & LoadedWholesaleUnitOfMeasurement & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@WholesaleUnitOfMeasurement", WholesaleUnitOfMeasurementTextBox.Text)
            End With
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "WholesaleUnitsOfMeasurement" & " With ID (" & WholesaleUnitOfMeasurementTextBox.Text & ")", 0)
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that  have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
        ...
        If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                If PanelNew.Visible Then
                    ThePKey = WholesaleUnitOfMeasurementTextBox.Text
                Else
                    ThePKey = GridView1.GetFocusedDataRow.Item("WholesaleUnitOfMeasurement")
                End If

                Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                Dim Query As String = "DELETE FROM WholesaleUnitsOfMeasurement WHERE WholesaleUnitOfMeasurement = '" & ThePKey & "'"
                Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                ...
                CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "WholesaleUnitsOfMeasurement" & " With ID (" & ThePKey & ")", 0)
                LoadRecords()
                Dim TheNewPKey As String = ""
                If MyReportTable.Rows.Count > 0 Then
                    TheNewPKey = GridView1.GetFocusedDataRow.Item("WholesaleUnitOfMeasurement")
                End If
    ...
    'Private Sub WholesaleUnitOfMeasurementTextBox_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles WholesaleUnitOfMeasurementTextBox.TextChanged
    'DisplayRecordInfo(WholesaleUnitOfMeasurementTextBox.Text)
    'End Sub
    Public Sub DisplayRecordInfo(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = " SELECT WholesaleUnitOfMeasurement FROM WholesaleUnitsOfMeasurement WHERE WholesaleUnitOfMeasurement = '" & RecordID & "'"
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
            ...
            If MySingleReportTable.Rows.Count > 0 Then
                ...
                WholesaleUnitOfMeasurementTextBox.Text = MySingleReportTable.Rows(0).Item("WholesaleUnitOfMeasurement").ToString
                XtraTabControlRelated.Visible = True
                LoadInventoryRecords(WholesaleUnitOfMeasurementTextBox.Text)
                LoadedWholesaleUnitOfMeasurement = WholesaleUnitOfMeasurementTextBox.Text
...
#Region "Inventory"
    ...
    Private Sub cmdAddInventory_Click(sender As Object, e As EventArgs) Handles cmdAddInventory.Click
        Try
            ...
            MyForm.WholesaleUnitOfMeasurementGridBox.EditValue = Me.WholesaleUnitOfMeasurementTextBox.Text
        ...
        If RecordAccessLevel.Contains("Delete") Then
            If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
                Try
                    ...
                    CreateLog(CurrentUserID, FullNameofUser, "Delete of Inventory for " + WholesaleUnitOfMeasurementTextBox.Text, 0)
                    LoadInventoryRecords(WholesaleUnitOfMeasurementTextBox.Text)
...
#End Region
    ...
    '       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportWholesaleUnitsOfMeasurement
    '       MyDocument.SummaryInfo.ReportTitle = "Wholesale Units Of Measurement"
    '	      Dim MyTable As New DataTable
    '       Dim TableName As String = "WholesaleUnitsOfMeasurement"
    '       Dim Fields As String = "WholesaleUnitOfMeasurement"
    '       Dim Values As String = WholesaleUnitOfMeasurementTextBox.Text
    ...
    '           Dim QrCodeData As String = "WholesaleUnitsOfMeasurement Report - Printed on: " & DateTime.Today
    ...
    Private Sub SearchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SearchToolStripMenuItem.Click
        ...
        Try
            ...
            MySearch.FormName = "frmWholesaleUnitsOfMeasurement"
            MySearch.TableName = "WholesaleUnitsOfMeasurement"
            MySearch.PrimaryKey = "WholesaleUnitOfMeasurement"
    ...
    Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("WholesaleUnitsOfMeasurement")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("WholesaleUnitsOfMeasurement")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub OpenToolStripButton_Click() Handles OpenToolStripButton.Click
        Try
            Dim DRecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("WholesaleUnitOfMeasurement"))
    ...
    Private Sub SearchToolStripButton_Click() Handles SearchToolStripButton.Click
        ...
        Try
            Dim QueryItems As String = "WholesaleUnitOfMeasurement"
            Dim TableName As String = "WholesaleUnitsOfMeasurement"
            ...
            If SearchToolStripComboBox.Text = "" Then
                Condition = ""
            Else
                Condition = "WholesaleUnitOfMeasurement = '" & SearchToolStripComboBox.Text & "'"
            End If

            Dim OrderByCondition As String = "ORDER BY WholesaleUnitOfMeasurement DESC"
            ...
            Else
                OpenNEWToolStripButton_Click()
                DisplayRecordInfo(MyTable.Rows(0).Item("WholesaleUnitOfMeasurement"))
            End If
            isDateSearchUsed = False
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub MySearchItemsLoading() Handles Me.Load
        Dim QueryItem As String = "WholesaleUnitOfMeasurement"
        Dim TableName As String = "WholesaleUnitsOfMeasurement"
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "WholesaleUnitsOfMeasurement", "WholesaleUnitOfMeasurement = '" & DRecordID & "'", " ORDER BY WholesaleUnitOfMeasurement ASC")
...
#Region "EXPORTING AND OTHER FUNCTIONS FROM DASHBOARD BUTTONS"
    ...
    Private Sub frmWholesaleUnitsOfMeasurement_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
#End Region
    ...
    Private Sub PrintToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintToolStripMenuItem.Click
        Try
            ...
            If PanelNew.Visible = False Then
                Dim RecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("WholesaleUnitOfMeasurement"))
            ...
            Dim MyDocument As New XtraReportWholesaleUnitsOfMeasurementSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.WholesaleUnitsOfMeasurementDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "Wholesale Units Of Measurement"
    ...
    Private Sub PrintAllListedRecordsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintAllListedRecordsToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportWholesaleUnitsOfMeasurementMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.WholesaleUnitsOfMeasurementDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "WholesaleUnitsOfMeasurement"

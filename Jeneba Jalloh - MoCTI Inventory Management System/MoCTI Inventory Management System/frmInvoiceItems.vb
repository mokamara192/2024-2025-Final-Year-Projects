...
Public Class frmTransferItems
    ...
    Private Sub LoadRecords()
        Try
            ...
            Dim Query As String = " SELECT ItemID,GeneratedTransferNo,StockID,SalesCategory,Quantity,UnitCost,TotalCost,UnitOfMeasurement,RegDate,InputedBy FROM TransferItems ORDER BY ItemID DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

            MyReportTable.Rows.Clear()
            ConTableAdapter.Fill(MyReportTable)

            TransferItemsDataGridView.DataSource = MyReportTable
    ...
    Private Function ValidateID(ByVal ItemID As String) As Boolean
        Try
            ...
            Dim Query As String = "SELECT Count(*) FROM TransferItems WHERE ItemID = '" & ItemID & "'"
    ...
    Private Function GenerateID() As String
        Try
            ...
            Dim Query As String = "SELECT Count(*) FROM TransferItems WHERE ItemID LIKE 'I" & DateTime.Today.ToString("yyMM") & "%'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
    ...
    Private Sub cmdAdd_Click() 'Handles cmdAdd.Click
        ...
        If Not isCalled Then
            MessageBox.Show("Sorry. Transfer Items CANNOT be added here. Please use the Transfer Form and under a particular Transfer to add items.", "NOT ALLOWED", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Exit Sub
        End If
    ...
    Private Sub cmdSave_Click() 'Handles cmdSave.Click
        Try
            ...
            Dim Query As String = "INSERT INTO TransferItems (ItemID,GeneratedTransferNo,StockID,SalesCategory,Quantity,UnitCost,TotalCost,UnitOfMeasurement,RegDate,InputedBy) VALUES(@ItemID,@GeneratedTransferNo,@StockID,@SalesCategory,@Quantity,@UnitCost,@TotalCost,@UnitOfMeasurement,@RegDate,@InputedBy)"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            ...
            UPDATETransferTotalAmount(GeneratedTransferNoGridBox.EditValue)

            lblInformation.Text = "Successfully Inserted a new record"
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "TransferItems" & " With ID (" & ItemIDTextBox.Text & ")", 0)
        ...
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that StockID:Inventory:StockID,GeneratedTransferNo:Transfers:GeneratedTransferNo have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
    ...
    Private Sub cmdUpdate_Click() 'Handles cmdUpdate.Click
        Try
            ...
            Dim Query As String = "UPDATE TransferItems SET GeneratedTransferNo =@GeneratedTransferNo, StockID =@StockID, SalesCategory =@SalesCategory, Quantity =@Quantity, UnitCost =@UnitCost, TotalCost =@TotalCost, UnitOfMeasurement =@UnitOfMeasurement, RegDate =@RegDate, InputedBy =@InputedBy WHERE ItemID = '" & LoadedItemID & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@ItemID", ItemIDTextBox.Text)
                .AddWithValue("@GeneratedTransferNo", GeneratedTransferNoGridBox.EditValue)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "TransferItems" & " With ID (" & ItemIDTextBox.Text & ")", 0)
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that StockID:Inventory:StockID,GeneratedTransferNo:Transfers:GeneratedTransferNo have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
    ...
    Private Sub cmdDelete_Click() 'Handles cmdDelete.Click
        If Not isCalled Then
            MessageBox.Show("Sorry. Transfer Items CANNOT be removed here. Please use the Transfer Form and under a particular Transfer to add items.", "NOT ALLOWED", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Exit Sub
        End If
        ...
        If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                Dim Query As String = "DELETE FROM TransferItems WHERE ItemID = '" & ThePKey & "'"
                Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                ...
                CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "TransferItems" & " With ID (" & ThePKey & ")", 0)
                LoadRecords()
    ...
    Private Sub UPDATETransferTotalAmount(ByVal GeneratedTransferNo As String)
        Try
            ...
            Dim Query As String = "UPDATE Transfers SET TotalAmount = @TotalAmount WHERE GeneratedTransferNo = '" & GeneratedTransferNo & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
    ...
    Private Sub LoadStockID()
        ...
        Try
            Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
            Dim Query As String = "Select StockID,NameOfItem,ItemCategory,RetailUnitPrice,WholesaleUnitPrice From Inventory ORDER BY StockID DESC"
    ...
    Public Sub DisplayRecordInfo(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = " SELECT ItemID,GeneratedTransferNo,StockID,SalesCategory,Quantity,UnitCost,TotalCost,UnitOfMeasurement,RegDate,InputedBy FROM TransferItems WHERE ItemID = '" & RecordID & "'"
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
    ...
    '       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportTransferItems
    '       MyDocument.SummaryInfo.ReportTitle = "Transfer Items"
    '	      Dim MyTable As New DataTable
    '       Dim TableName As String = "TransferItems"
    ...
    '           Dim QrCodeData As String = "TransferItems Report - Printed on: " & DateTime.Today
    ...
    Private Sub SearchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SearchToolStripMenuItem.Click
        ...
        Try
            ...
            MySearch.FormName = "frmTransferItems"
            MySearch.TableName = "TransferItems"
    ...
    Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("TransferItems")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("TransferItems")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub SearchToolStripButton_Click() Handles SearchToolStripButton.Click
        ...
        Try
            Dim QueryItems As String = "ItemID,GeneratedTransferNo,StockID,SalesCategory,Quantity,UnitCost,TotalCost,UnitOfMeasurement,RegDate,InputedBy"
            Dim TableName As String = "TransferItems"
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "TransferItems", "ItemID = '" & DRecordID & "'", " ORDER BY ItemID ASC")
...
#Region "EXPORTING AND OTHER FUNCTIONS FROM DASHBOARD BUTTONS"
    ...
    Private Sub frmTransferItems_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
#End Region
    ...
    Private Sub PrintToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportTransferItemsSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.TransferItemsDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "Transfer Items"
    ...
    Private Sub PrintAllListedRecordsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintAllListedRecordsToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportTransferItemsMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.TransferItemsDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "TransferItems"
...
#Region "SEARCH BY DATE"
    ...
    Private Sub FormLoadingSetup() Handles Me.Load
        Try
            ...
            Dim Allfields As String = GetAllFields("TransferItems", DatabaseName)
            ...
            For Each field In splitfields
                ...
                Dim ColumnType As String = GetColumnDataType("TransferItems", field).ToUpper
    ...
    Private Sub cmdDateSearch_Click(sender As Object, e As EventArgs) Handles cmdDateSearch.Click
        If SearchDateFieldComboBox.SelectedIndex <> -1 Then
            Try
                ...
                Dim Query As String = " SELECT ItemID,GeneratedTransferNo,StockID,SalesCategory,Quantity,UnitCost,TotalCost,UnitOfMeasurement,RegDate,InputedBy FROM TransferItems WHERE " & SearchDateFieldComboBox.Text & " BETWEEN '" & StartDateTimePicker.DateTime.ToString("yyy-MM-dd") & "' AND '" & EndDateTimePicker.DateTime.ToString("yyyy-MM-dd") & "' ORDER BY ItemID DESC "
                Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
    ...
    Private Sub PanelNew_Paint(sender As Object, e As PaintEventArgs) Handles PanelNew.Paint

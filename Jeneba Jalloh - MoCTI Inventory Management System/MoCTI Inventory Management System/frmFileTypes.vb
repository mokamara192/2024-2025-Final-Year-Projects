...
Public Class frmFileTypes
    ...
    Private Sub LoadRecords()
        Try
            ...
            Dim Query As String = " SELECT  FileType,OfficeType,RegDate,InputtedBy FROM FileTypes ORDER BY FileType DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

            MyReportTable.Rows.Clear()
            ConTableAdapter.Fill(MyReportTable)

            FileTypesDataGridView.DataSource = MyReportTable
    ...
    Private Function ValidateID(ByVal FileType As String) As Boolean
        Try
            ...
            Dim Query As String = "SELECT Count(*) FROM FileTypes WHERE FileType = '" & FileType & "'"
    ...
    Private Function GenerateID() As String
        Try
            ...
            Dim Query As String = "SELECT Count(*) FROM FileTypes WHERE FileType LIKE 'FIL" & DateTime.Today.ToString("yyMMdd") & "%'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
    ...
    Private Sub cmdSave_Click() 'Handles cmdSave.Click
        Try
            ...
            Dim Query As String = "INSERT INTO FileTypes (FileType,OfficeType,RegDate,InputtedBy) VALUES(@FileType,@OfficeType,@RegDate,@InputtedBy)"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "FileTypes" & " With ID (" & FileTypeTextBox.Text & ")", 0)
            LoadRecords()
            'cmdAdd_Click()
    ...
    Private Sub cmdUpdate_Click() 'Handles cmdUpdate.Click
        Try
            ...
            Dim Query As String = "UPDATE FileTypes SET OfficeType =@OfficeType, RegDate =@RegDate, InputtedBy =@InputtedBy WHERE FileType = '" & LoadedFileType & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@FileType", FileTypeTextBox.Text)
                .AddWithValue("@OfficeType", OfficeTypeTextBox.Text)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "FileTypes" & " With ID (" & FileTypeTextBox.Text & ")", 0)
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that  have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
    ...
    Private Sub cmdDelete_Click() 'Handles cmdDelete.Click
        If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                Dim Query As String = "DELETE FROM FileTypes WHERE FileType = '" & ThePKey & "'"
                Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                ...
                CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "FileTypes" & " With ID (" & ThePKey & ")", 0)
                LoadRecords()
    ...
    Public Sub DisplayRecordInfo(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = " SELECT FileType,OfficeType,RegDate,InputtedBy FROM FileTypes WHERE FileType = '" & RecordID & "'"
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
    ...
    '       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportFileTypes
    '       MyDocument.SummaryInfo.ReportTitle = "File Types"
    '	      Dim MyTable As New DataTable
    '       Dim TableName As String = "FileTypes"
    ...
    '           Dim QrCodeData As String = "FileTypes Report - Printed on: " & DateTime.Today
    ...
    Private Sub SearchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SearchToolStripMenuItem.Click
        ...
        Try
            ...
            MySearch.FormName = "frmFileTypes"
            MySearch.TableName = "FileTypes"
    ...
    Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("FileTypes")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("FileTypes")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "FileTypes", "FileType = '" & DRecordID & "'", " ORDER BY FileType ASC")
...
#Region "EXPORTING AND OTHER FUNCTIONS FROM DASHBOARD BUTTONS"
    ...
    Private Sub frmFileTypes_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
#End Region
    ...
    Private Sub PrintToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportFileTypesSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.FileTypesDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "File Types"
    ...
    Private Sub PrintAllListedRecordsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintAllListedRecordsToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportFileTypesMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.FileTypesDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "FileTypes"
...
#Region "SEARCH BY DATE"
    ...
    Private Sub FormLoadingSetup() Handles Me.Load
        Try
            ...
            Dim Allfields As String = GetAllFields("FileTypes", DatabaseName)
            ...
            For Each field In splitfields
                ...
                Dim ColumnType As String = GetColumnDataType("FileTypes", field).ToUpper
    ...
    Private Sub cmdDateSearch_Click(sender As Object, e As EventArgs) Handles cmdDateSearch.Click
        If SearchDateFieldComboBox.SelectedIndex <> -1 Then
            Try
                ...
                Dim Query As String = " SELECT FileType,OfficeType,RegDate,InputtedBy FROM FileTypes WHERE " & SearchDateFieldComboBox.Text & " BETWEEN '" & StartDateTimePicker.DateTime.ToString("yyy-MM-dd") & "' AND '" & EndDateTimePicker.DateTime.ToString("yyyy-MM-dd") & "' ORDER BY FileType DESC "
                Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

...
Public Class frmCurrencySetup
    ...
    Private Sub LoadRecords()
        Try
            ...
            Dim Query As String = " SELECT  Currency,ExchangeRate,CurrencySymbol FROM CurrencySetup ORDER BY Currency DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

            MyReportTable.Rows.Clear()
            ConTableAdapter.Fill(MyReportTable)

            CurrencySetupDataGridView.DataSource = MyReportTable
            isDateSearchUsed = False
            'If MyReportTable.Rows.Count > 0 Then
            'DisplayRecordInfo(MyReportTable.Rows(0).Item("Currency"))
            'End If
    ...
    Private Function ValidateID(ByVal Currency As String) As Boolean
        Try
            Dim Conn As New SqlClient.SqlConnection(MyConnectionString)

            Dim Query As String = "SELECT Count(*) FROM CurrencySetup WHERE Currency = '" & Currency & "'"
    ...
    Private Function GenerateID() As String
        Try
            ...
            Dim Query As String = "SELECT Count(*) FROM CurrencySetup WHERE Currency LIKE 'CUR" & DateTime.Today.ToString("yyMMdd") & "%'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            ...
            While IsIDGenerated = False
                ...
                If Recordcount >= 0 And Recordcount < 10 Then
                    Start = "CUR" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "000" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 10 And Recordcount < 100 Then
                    Start = "CUR" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "00" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 100 And Recordcount < 1000 Then
                    Start = "CUR" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "0" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 1000 And Recordcount < 10000 Then
                    Start = "CUR" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
    ...
    Private Sub cmdAdd_Click() 'Handles cmdAdd.Click
        ...
        'CurrencyTextBox.Text = GenerateID()

    End Sub

    Private Sub cmdSave_Click() 'Handles cmdSave.Click
        Try
            If CurrencyTextBox.Text <> "" Then
                If ValidateID(CurrencyTextBox.Text) Then
                    MessageBox.Show("This Currency already exist in this table. DUPLICATE 'CURRENCY' ARE NOT ALLOWED. Please verify and try again.", "Already Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "INSERT INTO CurrencySetup (Currency,ExchangeRate,CurrencySymbol) VALUES(@Currency,@ExchangeRate,@CurrencySymbol)"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@Currency", CurrencyTextBox.Text)
                .AddWithValue("@ExchangeRate", ExchangeRateTextBox.Text)
                .AddWithValue("@CurrencySymbol", CurrencySymbolTextBox.Text)
            End With
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "CurrencySetup" & " With ID (" & CurrencyTextBox.Text & ")", 0)
            LoadRecords()
            'cmdAdd_Click()
            DisplayRecordInfo(CurrencyTextBox.Text)
    ...
    Private Sub cmdUpdate_Click() 'Handles cmdUpdate.Click
        Try
            If Not ValidateID(LoadedCurrency) Then
                MessageBox.Show("The loaded Currency (" & LoadedCurrency & ") was not found in the database table. UPDATE OPERATION WILL NOT BE SUCCESSFULL. Please verify and try again.", "Not Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "UPDATE CurrencySetup SET ExchangeRate =@ExchangeRate, CurrencySymbol =@CurrencySymbol WHERE Currency = '" & LoadedCurrency & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@Currency", CurrencyTextBox.Text)
                .AddWithValue("@ExchangeRate", ExchangeRateTextBox.Text)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "CurrencySetup" & " With ID (" & CurrencyTextBox.Text & ")", 0)
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that  have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
        ...
        If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                If PanelNew.Visible Then
                    ThePKey = CurrencyTextBox.Text
                Else
                    ThePKey = GridView1.GetFocusedDataRow.Item("Currency")
                End If

                Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                Dim Query As String = "DELETE FROM CurrencySetup WHERE Currency = '" & ThePKey & "'"
                Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                ...
                CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "CurrencySetup" & " With ID (" & ThePKey & ")", 0)
                LoadRecords()
                Dim TheNewPKey As String = ""
                If MyReportTable.Rows.Count > 0 Then
                    TheNewPKey = GridView1.GetFocusedDataRow.Item("Currency")
                End If
    ...
    'Private Sub CurrencyTextBox_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles CurrencyTextBox.TextChanged
    'DisplayRecordInfo(CurrencyTextBox.Text)
    'End Sub
    Public Sub DisplayRecordInfo(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = " SELECT Currency,ExchangeRate,CurrencySymbol FROM CurrencySetup WHERE Currency = '" & RecordID & "'"
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
            ...
            If MySingleReportTable.Rows.Count > 0 Then
                ...
                CurrencyTextBox.Text = MySingleReportTable.Rows(0).Item("Currency").ToString
                ExchangeRateTextBox.Text = MySingleReportTable.Rows(0).Item("ExchangeRate").ToString
                CurrencySymbolTextBox.Text = MySingleReportTable.Rows(0).Item("CurrencySymbol").ToString
                ' XtraTabControlRelated.Visible = True
                'LoadTransfersRecords(CurrencyTextBox.Text)
                LoadedCurrency = CurrencyTextBox.Text
    ...
    '       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportCurrencySetup
    '       MyDocument.SummaryInfo.ReportTitle = "Currency Setup"
    '	      Dim MyTable As New DataTable
    '       Dim TableName As String = "CurrencySetup"
    '       Dim Fields As String = "Currency"
    '       Dim Values As String = CurrencyTextBox.Text
    ...
    '           Dim QrCodeData As String = "CurrencySetup Report - Printed on: " & DateTime.Today
    ...
    Private Sub SearchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SearchToolStripMenuItem.Click
        ...
        Try
            ...
            MySearch.FormName = "frmCurrencySetup"
            MySearch.TableName = "CurrencySetup"
            MySearch.PrimaryKey = "Currency"
    ...
    Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("CurrencySetup")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("CurrencySetup")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub OpenToolStripButton_Click() Handles OpenToolStripButton.Click
        Try
            Dim DRecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("Currency"))
    ...
    Private Sub SearchToolStripButton_Click() Handles SearchToolStripButton.Click
        ...
        Try
            Dim QueryItems As String = "Currency,ExchangeRate,CurrencySymbol"
            Dim TableName As String = "CurrencySetup"
            ...
            If SearchToolStripComboBox.Text = "" Then
                Condition = ""
            Else
                Condition = "Currency = '" & SearchToolStripComboBox.Text & "'"
            End If

            Dim OrderByCondition As String = "ORDER BY Currency DESC"
            ...
            Else
                OpenNEWToolStripButton_Click()
                DisplayRecordInfo(MyTable.Rows(0).Item("Currency"))
            End If
            isDateSearchUsed = False
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub MySearchItemsLoading() Handles Me.Load
        Dim QueryItem As String = "Currency"
        Dim TableName As String = "CurrencySetup"
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "CurrencySetup", "Currency = '" & DRecordID & "'", " ORDER BY Currency ASC")
...
#Region "EXPORTING AND OTHER FUNCTIONS FROM DASHBOARD BUTTONS"
    ...
    Private Sub frmCurrencySetup_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
#End Region
    ...
    Private Sub PrintToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintToolStripMenuItem.Click
        Try
            ...
            If PanelNew.Visible = False Then
                Dim RecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("Currency"))
            ...
            Dim MyDocument As New XtraReportCurrencySetupSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.CurrencySetupDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "Currency Setup"
    ...
    Private Sub PrintAllListedRecordsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintAllListedRecordsToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportCurrencySetupMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.CurrencySetupDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "CurrencySetup"

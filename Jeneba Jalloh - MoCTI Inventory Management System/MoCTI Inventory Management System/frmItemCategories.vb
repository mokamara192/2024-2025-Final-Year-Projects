...
Public Class frmItemCategories
    ...
    Dim LoadedItemCategory As String
    ...
    Private Sub LoadRecords()
        Try
            ...
            Dim Query As String = " SELECT  ItemCategory FROM ItemCategories ORDER BY ItemCategory DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

            MyReportTable.Rows.Clear()
            ConTableAdapter.Fill(MyReportTable)

            ItemCategoriesDataGridView.DataSource = MyReportTable
            isDateSearchUsed = False
            'If MyReportTable.Rows.Count > 0 Then
            'DisplayRecordInfo(MyReportTable.Rows(0).Item("ItemCategory"))
            'End If
    ...
    Private Function ValidateID(ByVal ItemCategory As String) As Boolean
        Try
            Dim Conn As New SqlClient.SqlConnection(MyConnectionString)

            Dim Query As String = "SELECT Count(*) FROM ItemCategories WHERE ItemCategory = '" & ItemCategory & "'"
    ...
    Private Function GenerateID() As String
        Try
            ...
            Dim Query As String = "SELECT Count(*) FROM ItemCategories WHERE ItemCategory LIKE 'ITE" & DateTime.Today.ToString("yyMMdd") & "%'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            ...
            While IsIDGenerated = False
                ...
                If Recordcount >= 0 And Recordcount < 10 Then
                    Start = "ITE" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "000" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 10 And Recordcount < 100 Then
                    Start = "ITE" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "00" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 100 And Recordcount < 1000 Then
                    Start = "ITE" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "0" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 1000 And Recordcount < 10000 Then
                    Start = "ITE" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
    ...
    Private Sub cmdAdd_Click() 'Handles cmdAdd.Click
        ...
        If Not isCalled Then
            LoadedItemCategory = ""
        End If
        ItemCategoryTextBox.Text = ""

        'ItemCategoryTextBox.Text = "Auto"

    End Sub

    Private Sub cmdSave_Click() 'Handles cmdSave.Click
        Try
            If ItemCategoryTextBox.Text <> "" Then
                If ValidateID(ItemCategoryTextBox.Text) Then
                    MessageBox.Show("This Item Category already exist in this table. DUPLICATE 'ITEM CATEGORY' ARE NOT ALLOWED. Please verify and try again.", "Already Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "INSERT INTO ItemCategories (ItemCategory) VALUES(@ItemCategory)"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@ItemCategory", ItemCategoryTextBox.Text)
            End With
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "ItemCategories" & " With ID (" & ItemCategoryTextBox.Text & ")", 0)
            LoadRecords()
            'cmdAdd_Click()
            DisplayRecordInfo(ItemCategoryTextBox.Text)
    ...
    Private Sub cmdUpdate_Click() 'Handles cmdUpdate.Click
        Try
            If Not ValidateID(LoadedItemCategory) Then
                MessageBox.Show("The loaded Item Category (" & LoadedItemCategory & ") was not found in the database table. UPDATE OPERATION WILL NOT BE SUCCESSFULL. Please verify and try again.", "Not Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "UPDATE ItemCategories SET  WHERE ItemCategory = '" & LoadedItemCategory & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@ItemCategory", ItemCategoryTextBox.Text)
            End With
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "ItemCategories" & " With ID (" & ItemCategoryTextBox.Text & ")", 0)
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that  have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
        ...
        If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                If PanelNew.Visible Then
                    ThePKey = ItemCategoryTextBox.Text
                Else
                    ThePKey = GridView1.GetFocusedDataRow.Item("ItemCategory")
                End If

                Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                Dim Query As String = "DELETE FROM ItemCategories WHERE ItemCategory = '" & ThePKey & "'"
                Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                ...
                CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "ItemCategories" & " With ID (" & ThePKey & ")", 0)
                LoadRecords()
                Dim TheNewPKey As String = ""
                If MyReportTable.Rows.Count > 0 Then
                    TheNewPKey = GridView1.GetFocusedDataRow.Item("ItemCategory")
                End If
    ...
    'Private Sub ItemCategoryTextBox_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles ItemCategoryTextBox.TextChanged
    'DisplayRecordInfo(ItemCategoryTextBox.Text)
    'End Sub
    Public Sub DisplayRecordInfo(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = " SELECT ItemCategory FROM ItemCategories WHERE ItemCategory = '" & RecordID & "'"
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
            ...
            If MySingleReportTable.Rows.Count > 0 Then
                ...
                ItemCategoryTextBox.Text = MySingleReportTable.Rows(0).Item("ItemCategory").ToString
                XtraTabControlRelated.Visible = True
                LoadInventoryRecords(ItemCategoryTextBox.Text)
                LoadedItemCategory = ItemCategoryTextBox.Text
...
#Region "Inventory"
    ...
    Private Sub cmdAddInventory_Click(sender As Object, e As EventArgs) Handles cmdAddInventory.Click
        Try
            ...
            MyForm.ItemCategoryGridBox.EditValue = Me.ItemCategoryTextBox.Text
        ...
        If RecordAccessLevel.Contains("Delete") Then
            If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
                Try
                    ...
                    CreateLog(CurrentUserID, FullNameofUser, "Delete of Inventory for " + ItemCategoryTextBox.Text, 0)
                    LoadInventoryRecords(ItemCategoryTextBox.Text)
...
#End Region
    ...
    '       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportItemCategories
    '       MyDocument.SummaryInfo.ReportTitle = "Item Categories"
    '	      Dim MyTable As New DataTable
    '       Dim TableName As String = "ItemCategories"
    '       Dim Fields As String = "ItemCategory"
    '       Dim Values As String = ItemCategoryTextBox.Text
    ...
    '           Dim QrCodeData As String = "ItemCategories Report - Printed on: " & DateTime.Today
    ...
    Private Sub SearchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SearchToolStripMenuItem.Click
        ...
        Try
            ...
            MySearch.FormName = "frmItemCategories"
            MySearch.TableName = "ItemCategories"
            MySearch.PrimaryKey = "ItemCategory"
    ...
    Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("ItemCategories")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("ItemCategories")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub OpenToolStripButton_Click() Handles OpenToolStripButton.Click
        Try
            Dim DRecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("ItemCategory"))
    ...
    Private Sub SearchToolStripButton_Click() Handles SearchToolStripButton.Click
        ...
        Try
            Dim QueryItems As String = "ItemCategory"
            Dim TableName As String = "ItemCategories"
            ...
            If SearchToolStripComboBox.Text = "" Then
                Condition = ""
            Else
                Condition = "ItemCategory = '" & SearchToolStripComboBox.Text & "'"
            End If

            Dim OrderByCondition As String = "ORDER BY ItemCategory DESC"
            ...
            Else
                OpenNEWToolStripButton_Click()
                DisplayRecordInfo(MyTable.Rows(0).Item("ItemCategory"))
            End If
            isDateSearchUsed = False
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub MySearchItemsLoading() Handles Me.Load
        Dim QueryItem As String = "ItemCategory"
        Dim TableName As String = "ItemCategories"
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "ItemCategories", "ItemCategory = '" & DRecordID & "'", " ORDER BY ItemCategory ASC")
...
#Region "EXPORTING AND OTHER FUNCTIONS FROM DASHBOARD BUTTONS"
    ...
    Private Sub frmItemCategories_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
#End Region
    ...
    Private Sub PrintToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintToolStripMenuItem.Click
        Try
            ...
            If PanelNew.Visible = False Then
                Dim RecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("ItemCategory"))
            ...
            Dim MyDocument As New XtraReportItemCategoriesSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.ItemCategoriesDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "Item Categories"
    ...
    Private Sub PrintAllListedRecordsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintAllListedRecordsToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportItemCategoriesMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.ItemCategoriesDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "ItemCategories"

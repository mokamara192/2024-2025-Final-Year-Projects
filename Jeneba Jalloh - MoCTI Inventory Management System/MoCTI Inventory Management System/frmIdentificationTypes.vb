...
Public Class frmIdentificationTypes
    ...
    Dim LoadedIdentificationType As String
    ...
    Private Sub LoadRecords()
        Try
            ...
            Dim Query As String = " SELECT  IdentificationType,RegDate,InputtedBy FROM IdentificationTypes ORDER BY IdentificationType DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

            MyReportTable.Rows.Clear()
            ConTableAdapter.Fill(MyReportTable)

            IdentificationTypesDataGridView.DataSource = MyReportTable
            isDateSearchUsed = False
            'If MyReportTable.Rows.Count > 0 Then
            'DisplayRecordInfo(MyReportTable.Rows(0).Item("IdentificationType"))
            'End If
    ...
    Private Function ValidateID(ByVal IdentificationType As String) As Boolean
        Try
            Dim Conn As New SqlClient.SqlConnection(MyConnectionString)

            Dim Query As String = "SELECT Count(*) FROM IdentificationTypes WHERE IdentificationType = '" & IdentificationType & "'"
    ...
    Private Function GenerateID() As String
        Try
            ...
            Dim Query As String = "SELECT Count(*) FROM IdentificationTypes WHERE IdentificationType LIKE 'IDE" & DateTime.Today.ToString("yyMMdd") & "%'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            ...
            While IsIDGenerated = False
                ...
                If Recordcount >= 0 And Recordcount < 10 Then
                    Start = "IDE" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "000" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 10 And Recordcount < 100 Then
                    Start = "IDE" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "00" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 100 And Recordcount < 1000 Then
                    Start = "IDE" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "0" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 1000 And Recordcount < 10000 Then
                    Start = "IDE" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
    ...
    Private Sub cmdAdd_Click() 'Handles cmdAdd.Click
        ...
        IdentificationTypeTextBox.Text = GenerateID()

    End Sub

    Private Sub cmdSave_Click() 'Handles cmdSave.Click
        Try
            If IdentificationTypeTextBox.Text <> "" Then
                If ValidateID(IdentificationTypeTextBox.Text) Then
                    MessageBox.Show("This Identification Type already exist in this table. DUPLICATE 'IDENTIFICATION TYPE' ARE NOT ALLOWED. Please verify and try again.", "Already Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "INSERT INTO IdentificationTypes (IdentificationType,RegDate,InputtedBy) VALUES(@IdentificationType,@RegDate,@InputtedBy)"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "IdentificationTypes" & " With ID (" & IdentificationTypeTextBox.Text & ")", 0)
            LoadRecords()
            'cmdAdd_Click()
            DisplayRecordInfo(IdentificationTypeTextBox.Text)
    ...
    Private Sub cmdUpdate_Click() 'Handles cmdUpdate.Click
        Try
            If Not ValidateID(LoadedIdentificationType) Then
                MessageBox.Show("The loaded Identification Type (" & LoadedIdentificationType & ") was not found in the database table. UPDATE OPERATION WILL NOT BE SUCCESSFULL. Please verify and try again.", "Not Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "UPDATE IdentificationTypes SET RegDate =@RegDate, InputtedBy =@InputtedBy WHERE IdentificationType = '" & LoadedIdentificationType & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@IdentificationType", IdentificationTypeTextBox.Text)
                .AddWithValue("@RegDate", Date.Today)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "IdentificationTypes" & " With ID (" & IdentificationTypeTextBox.Text & ")", 0)
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that  have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
        ...
        If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                If PanelNew.Visible Then
                    ThePKey = IdentificationTypeTextBox.Text
                Else
                    ThePKey = GridView1.GetFocusedDataRow.Item("IdentificationType")
                End If

                Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                Dim Query As String = "DELETE FROM IdentificationTypes WHERE IdentificationType = '" & ThePKey & "'"
                Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                ...
                CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "IdentificationTypes" & " With ID (" & ThePKey & ")", 0)
                LoadRecords()
                Dim TheNewPKey As String = ""
                If MyReportTable.Rows.Count > 0 Then
                    TheNewPKey = GridView1.GetFocusedDataRow.Item("IdentificationType")
                End If
    ...
    'Private Sub IdentificationTypeTextBox_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles IdentificationTypeTextBox.TextChanged
    'DisplayRecordInfo(IdentificationTypeTextBox.Text)
    'End Sub
    Public Sub DisplayRecordInfo(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = " SELECT IdentificationType,RegDate,InputtedBy FROM IdentificationTypes WHERE IdentificationType = '" & RecordID & "'"
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
            ...
            If MySingleReportTable.Rows.Count > 0 Then
                ...
                LoadedIdentificationType = IdentificationTypeTextBox.Text
    ...
    '       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportIdentificationTypes
    '       MyDocument.SummaryInfo.ReportTitle = "Identification Types"
    '	      Dim MyTable As New DataTable
    '       Dim TableName As String = "IdentificationTypes"
    '       Dim Fields As String = "IdentificationType"
    '       Dim Values As String = IdentificationTypeTextBox.Text
    ...
    '           Dim QrCodeData As String = "IdentificationTypes Report - Printed on: " & DateTime.Today
    ...
    Private Sub SearchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SearchToolStripMenuItem.Click
        ...
        Try
            ...
            MySearch.FormName = "frmIdentificationTypes"
            MySearch.TableName = "IdentificationTypes"
            MySearch.PrimaryKey = "IdentificationType"
    ...
    Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("IdentificationTypes")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("IdentificationTypes")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub OpenToolStripButton_Click() Handles OpenToolStripButton.Click
        Try
            Dim DRecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("IdentificationType"))
    ...
    Private Sub SearchToolStripButton_Click() Handles SearchToolStripButton.Click
        ...
        Try
            Dim QueryItems As String = "IdentificationType,RegDate,InputtedBy"
            Dim TableName As String = "IdentificationTypes"
            ...
            If SearchToolStripComboBox.Text = "" Then
                Condition = ""
            Else
                Condition = "IdentificationType = '" & SearchToolStripComboBox.Text & "'"
            End If

            Dim OrderByCondition As String = "ORDER BY IdentificationType DESC"
            ...
            Else
                OpenNEWToolStripButton_Click()
                DisplayRecordInfo(MyTable.Rows(0).Item("IdentificationType"))
            End If
            isDateSearchUsed = False
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub MySearchItemsLoading() Handles Me.Load
        Dim QueryItem As String = "IdentificationType"
        Dim TableName As String = "IdentificationTypes"
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "IdentificationTypes", "IdentificationType = '" & DRecordID & "'", " ORDER BY IdentificationType ASC")
...
#Region "EXPORTING AND OTHER FUNCTIONS FROM DASHBOARD BUTTONS"
    ...
    Private Sub frmIdentificationTypes_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
#End Region
    ...
    Private Sub PrintToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintToolStripMenuItem.Click
        Try
            ...
            If PanelNew.Visible = False Then
                Dim RecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("IdentificationType"))
            ...
            Dim MyDocument As New XtraReportIdentificationTypesSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.IdentificationTypesDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "Identification Types"
    ...
    Private Sub PrintAllListedRecordsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintAllListedRecordsToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportIdentificationTypesMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.IdentificationTypesDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "IdentificationTypes"
...
#Region "SEARCH BY DATE"
    ...
    Private Sub FormLoadingSetup() Handles Me.Load
        Try
            ...
            Dim Allfields As String = GetAllFields("IdentificationTypes", DatabaseName)
            ...
            For Each field In splitfields
                ...
                Dim ColumnType As String = GetColumnDataType("IdentificationTypes", field).ToUpper
    ...
    Private Sub cmdDateSearch_Click(sender As Object, e As EventArgs) Handles cmdDateSearch.Click
        If SearchDateFieldComboBox.SelectedIndex <> -1 Then
            Try
                ...
                Dim Query As String = " SELECT IdentificationType,RegDate,InputtedBy FROM IdentificationTypes WHERE " & SearchDateFieldComboBox.Text & " BETWEEN '" & StartDateTimePicker.DateTime.ToString("yyy-MM-dd") & "' AND '" & EndDateTimePicker.DateTime.ToString("yyyy-MM-dd") & "' ORDER BY IdentificationType DESC "
                Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

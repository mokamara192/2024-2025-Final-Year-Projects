...
Public Class frmOfficeTypes
    ...
    Dim LoadedOfficeType As String
    ...
    Private Sub LoadRecords()
        Try
            ...
            Dim Query As String = " SELECT  OfficeType,RegDate,InputtedBy FROM OfficeTypes ORDER BY OfficeType DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

            MyReportTable.Rows.Clear()
            ConTableAdapter.Fill(MyReportTable)

            OfficeTypesDataGridView.DataSource = MyReportTable
            isDateSearchUsed = False
            'If MyReportTable.Rows.Count > 0 Then
            'DisplayRecordInfo(MyReportTable.Rows(0).Item("OfficeType"))
            'End If
    ...
    Private Function ValidateID(ByVal OfficeType As String) As Boolean
        Try
            Dim Conn As New SqlClient.SqlConnection(MyConnectionString)

            Dim Query As String = "SELECT Count(*) FROM OfficeTypes WHERE OfficeType = '" & OfficeType & "'"
    ...
    Private Function GenerateID() As String
        Try
            ...
            Dim Query As String = "SELECT Count(*) FROM OfficeTypes WHERE OfficeType LIKE 'CUS" & DateTime.Today.ToString("yyMMdd") & "%'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
    ...
    Private Sub cmdAdd_Click() 'Handles cmdAdd.Click
        ...
        'OfficeTypeTextBox.Text = GenerateID()

    End Sub

    Private Sub cmdSave_Click() 'Handles cmdSave.Click
        Try
            If OfficeTypeTextBox.Text <> "" Then
                If ValidateID(OfficeTypeTextBox.Text) Then
                    MessageBox.Show("This Office Type already exist in this table. DUPLICATE 'Office TYPE' ARE NOT ALLOWED. Please verify and try again.", "Already Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "INSERT INTO OfficeTypes (OfficeType,RegDate,InputtedBy) VALUES(@OfficeType,@RegDate,@InputtedBy)"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "OfficeTypes" & " With ID (" & OfficeTypeTextBox.Text & ")", 0)
            LoadRecords()
            'cmdAdd_Click()
            DisplayRecordInfo(OfficeTypeTextBox.Text)
    ...
    Private Sub cmdUpdate_Click() 'Handles cmdUpdate.Click
        Try
            If Not ValidateID(LoadedOfficeType) Then
                MessageBox.Show("The loaded Office Type (" & LoadedOfficeType & ") was not found in the database table. UPDATE OPERATION WILL NOT BE SUCCESSFULL. Please verify and try again.", "Not Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "UPDATE OfficeTypes SET RegDate =@RegDate, InputtedBy =@InputtedBy WHERE OfficeType = '" & LoadedOfficeType & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@OfficeType", OfficeTypeTextBox.Text)
                .AddWithValue("@RegDate", Date.Today)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "OfficeTypes" & " With ID (" & OfficeTypeTextBox.Text & ")", 0)
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that  have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
        ...
        If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                If PanelNew.Visible Then
                    ThePKey = OfficeTypeTextBox.Text
                Else
                    ThePKey = GridView1.GetFocusedDataRow.Item("OfficeType")
                End If

                Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                Dim Query As String = "DELETE FROM OfficeTypes WHERE OfficeType = '" & ThePKey & "'"
                Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                ...
                CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "OfficeTypes" & " With ID (" & ThePKey & ")", 0)
                LoadRecords()
                Dim TheNewPKey As String = ""
                If MyReportTable.Rows.Count > 0 Then
                    TheNewPKey = GridView1.GetFocusedDataRow.Item("OfficeType")
                End If
    ...
    'Private Sub OfficeTypeTextBox_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles OfficeTypeTextBox.TextChanged
    'DisplayRecordInfo(OfficeTypeTextBox.Text)
    'End Sub
    Public Sub DisplayRecordInfo(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = " SELECT OfficeType,RegDate,InputtedBy FROM OfficeTypes WHERE OfficeType = '" & RecordID & "'"
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
            ...
            If MySingleReportTable.Rows.Count > 0 Then
                ...
                LoadedOfficeType = OfficeTypeTextBox.Text
    ...
    '       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportOfficeTypes
    '       MyDocument.SummaryInfo.ReportTitle = "Office Types"
    '	      Dim MyTable As New DataTable
    '       Dim TableName As String = "OfficeTypes"
    '       Dim Fields As String = "OfficeType"
    '       Dim Values As String = OfficeTypeTextBox.Text
    ...
    '           Dim QrCodeData As String = "OfficeTypes Report - Printed on: " & DateTime.Today
    ...
    Private Sub SearchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SearchToolStripMenuItem.Click
        ...
        Try
            ...
            MySearch.FormName = "frmOfficeTypes"
            MySearch.TableName = "OfficeTypes"
            MySearch.PrimaryKey = "OfficeType"
    ...
    Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("OfficeTypes")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("OfficeTypes")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub OpenToolStripButton_Click() Handles OpenToolStripButton.Click
        Try
            Dim DRecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("OfficeType"))
    ...
    Private Sub SearchToolStripButton_Click() Handles SearchToolStripButton.Click
        ...
        Try
            Dim QueryItems As String = "OfficeType,RegDate,InputtedBy"
            Dim TableName As String = "OfficeTypes"
            ...
            If SearchToolStripComboBox.Text = "" Then
                Condition = ""
            Else
                Condition = "OfficeType = '" & SearchToolStripComboBox.Text & "'"
            End If

            Dim OrderByCondition As String = "ORDER BY OfficeType DESC"
            ...
            Else
                OpenNEWToolStripButton_Click()
                DisplayRecordInfo(MyTable.Rows(0).Item("OfficeType"))
            End If
            isDateSearchUsed = False
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub MySearchItemsLoading() Handles Me.Load
        Dim QueryItem As String = "OfficeType"
        Dim TableName As String = "OfficeTypes"
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "OfficeTypes", "OfficeType = '" & DRecordID & "'", " ORDER BY OfficeType ASC")
...
#Region "EXPORTING AND OTHER FUNCTIONS FROM DASHBOARD BUTTONS"
    ...
    Private Sub frmOfficeTypes_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
#End Region
    ...
    Private Sub PrintToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintToolStripMenuItem.Click
        Try
            ...
            If PanelNew.Visible = False Then
                Dim RecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("OfficeType"))
            ...
            Dim MyDocument As New XtraReportOfficeTypesSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.OfficeTypesDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "Office Types"
    ...
    Private Sub PrintAllListedRecordsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintAllListedRecordsToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportOfficeTypesMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.OfficeTypesDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "OfficeTypes"
...
#Region "SEARCH BY DATE"
    ...
    Private Sub FormLoadingSetup() Handles Me.Load
        Try
            ...
            Dim Allfields As String = GetAllFields("OfficeTypes", DatabaseName)
            ...
            For Each field In splitfields
                ...
                Dim ColumnType As String = GetColumnDataType("OfficeTypes", field).ToUpper
    ...
    Private Sub cmdDateSearch_Click(sender As Object, e As EventArgs) Handles cmdDateSearch.Click
        If SearchDateFieldComboBox.SelectedIndex <> -1 Then
            Try
                ...
                Dim Query As String = " SELECT OfficeType,RegDate,InputtedBy FROM OfficeTypes WHERE " & SearchDateFieldComboBox.Text & " BETWEEN '" & StartDateTimePicker.DateTime.ToString("yyy-MM-dd") & "' AND '" & EndDateTimePicker.DateTime.ToString("yyyy-MM-dd") & "' ORDER BY OfficeType DESC "
                Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

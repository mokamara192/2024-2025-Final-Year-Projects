...
Public Class frmTransferFileTypes
    ...
    Private Sub LoadRecords()
        Try
            ...
            Dim Query As String = " SELECT  FileType,OfficeType,RegDate,InputtedBy FROM TransferFileTypes ORDER BY FileType DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

            MyReportTable.Rows.Clear()
            ConTableAdapter.Fill(MyReportTable)

            TransferFileTypesDataGridView.DataSource = MyReportTable
            isDateSearchUsed = False
            'If MyReportTable.Rows.Count > 0 Then
            'DisplayRecordInfo(MyReportTable.Rows(0).Item("FileType"))
            'End If
    ...
    Private Function ValidateID(ByVal FileType As String) As Boolean
        Try
            Dim Conn As New SqlClient.SqlConnection(MyConnectionString)

            Dim Query As String = "SELECT Count(*) FROM TransferFileTypes WHERE FileType = '" & FileType & "'"
    ...
    Private Function GenerateID() As String
        Try
            ...
            Dim Query As String = "SELECT Count(*) FROM TransferFileTypes WHERE FileType LIKE 'FIL" & DateTime.Today.ToString("yyMMdd") & "%'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            ...
            While IsIDGenerated = False
                ...
                If Recordcount >= 0 And Recordcount < 10 Then
                    Start = "FIL" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "000" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 10 And Recordcount < 100 Then
                    Start = "FIL" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "00" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 100 And Recordcount < 1000 Then
                    Start = "FIL" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "0" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 1000 And Recordcount < 10000 Then
                    Start = "FIL" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
    ...
    Private Sub cmdAdd_Click() 'Handles cmdAdd.Click
        ...
        'FileTypeTextBox.Text = GenerateID
    ...
    Private Sub cmdSave_Click() 'Handles cmdSave.Click
        Try
            If FileTypeTextBox.Text <> "" Then
                If ValidateID(FileTypeTextBox.Text) Then
                    MessageBox.Show("This File Type already exist in this table. DUPLICATE 'FILE TYPE' ARE NOT ALLOWED. Please verify and try again.", "Already Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "INSERT INTO TransferFileTypes (FileType,OfficeType,RegDate,InputtedBy) VALUES(@FileType,@OfficeType,@RegDate,@InputtedBy)"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "TransferFileTypes" & " With ID (" & FileTypeTextBox.Text & ")", 0)
            LoadRecords()
            'cmdAdd_Click()
            DisplayRecordInfo(FileTypeTextBox.Text)
    ...
    Private Sub cmdUpdate_Click() 'Handles cmdUpdate.Click
        Try
            If Not ValidateID(LoadedFileType) Then
                MessageBox.Show("The loaded File Type (" & LoadedFileType & ") was not found in the database table. UPDATE OPERATION WILL NOT BE SUCCESSFULL. Please verify and try again.", "Not Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "UPDATE TransferFileTypes SET OfficeType =@OfficeType, RegDate =@RegDate, InputtedBy =@InputtedBy WHERE FileType = '" & LoadedFileType & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@FileType", FileTypeTextBox.Text)
                .AddWithValue("@OfficeType", OfficeTypeTextBox.Text)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "TransferFileTypes" & " With ID (" & FileTypeTextBox.Text & ")", 0)
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that  have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
        ...
        If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                If PanelNew.Visible Then
                    ThePKey = FileTypeTextBox.Text
                Else
                    ThePKey = GridView1.GetFocusedDataRow.Item("FileType")
                End If

                Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                Dim Query As String = "DELETE FROM TransferFileTypes WHERE FileType = '" & ThePKey & "'"
                Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                ...
                CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "TransferFileTypes" & " With ID (" & ThePKey & ")", 0)
                LoadRecords()
                Dim TheNewPKey As String = ""
                If MyReportTable.Rows.Count > 0 Then
                    TheNewPKey = GridView1.GetFocusedDataRow.Item("FileType")
                End If
    ...
    'Private Sub FileTypeTextBox_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles FileTypeTextBox.TextChanged
    'DisplayRecordInfo(FileTypeTextBox.Text)
    'End Sub
    Public Sub DisplayRecordInfo(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = " SELECT FileType,OfficeType,RegDate,InputtedBy FROM TransferFileTypes WHERE FileType = '" & RecordID & "'"
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
            ...
            If MySingleReportTable.Rows.Count > 0 Then
                ...
                LoadedFileType = FileTypeTextBox.Text
    ...
    '       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportTransferFileTypes
    '       MyDocument.SummaryInfo.ReportTitle = "Transfer File Types"
    '	      Dim MyTable As New DataTable
    '       Dim TableName As String = "TransferFileTypes"
    '       Dim Fields As String = "FileType"
    '       Dim Values As String = FileTypeTextBox.Text
    ...
    '           Dim QrCodeData As String = "TransferFileTypes Report - Printed on: " & DateTime.Today
    ...
    Private Sub SearchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SearchToolStripMenuItem.Click
        ...
        Try
            ...
            MySearch.FormName = "frmTransferFileTypes"
            MySearch.TableName = "TransferFileTypes"
            MySearch.PrimaryKey = "FileType"
    ...
    Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("TransferFileTypes")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("TransferFileTypes")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub OpenToolStripButton_Click() Handles OpenToolStripButton.Click
        Try
            Dim DRecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("FileType"))
    ...
    Private Sub SearchToolStripButton_Click() Handles SearchToolStripButton.Click
        ...
        Try
            Dim QueryItems As String = "FileType,OfficeType,RegDate,InputtedBy"
            Dim TableName As String = "TransferFileTypes"
            ...
            If SearchToolStripComboBox.Text = "" Then
                Condition = ""
            Else
                Condition = "FileType = '" & SearchToolStripComboBox.Text & "'"
            End If

            Dim OrderByCondition As String = "ORDER BY FileType DESC"
            ...
            Else
                OpenNEWToolStripButton_Click()
                DisplayRecordInfo(MyTable.Rows(0).Item("FileType"))
            End If
            isDateSearchUsed = False
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub MySearchItemsLoading() Handles Me.Load
        Dim QueryItem As String = "FileType"
        Dim TableName As String = "TransferFileTypes"
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "TransferFileTypes", "FileType = '" & DRecordID & "'", " ORDER BY FileType ASC")
...
#Region "EXPORTING AND OTHER FUNCTIONS FROM DASHBOARD BUTTONS"
    ...
    Private Sub frmTransferFileTypes_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
#End Region
    ...
    Private Sub PrintToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintToolStripMenuItem.Click
        Try
            ...
            If PanelNew.Visible = False Then
                Dim RecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("FileType"))
            ...
            Dim MyDocument As New XtraReportTransferFileTypesSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.TransferFileTypesDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "Transfer File Types"
    ...
    Private Sub PrintAllListedRecordsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintAllListedRecordsToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportTransferFileTypesMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.TransferFileTypesDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "TransferFileTypes"
...
#Region "SEARCH BY DATE"
    ...
    Private Sub FormLoadingSetup() Handles Me.Load
        Try
            ...
            Dim Allfields As String = GetAllFields("TransferFileTypes", DatabaseName)
            ...
            For Each field In splitfields
                ...
                Dim ColumnType As String = GetColumnDataType("TransferFileTypes", field).ToUpper
    ...
    Private Sub cmdDateSearch_Click(sender As Object, e As EventArgs) Handles cmdDateSearch.Click
        If SearchDateFieldComboBox.SelectedIndex <> -1 Then
            Try
                ...
                Dim Query As String = " SELECT FileType,OfficeType,RegDate,InputtedBy FROM TransferFileTypes WHERE " & SearchDateFieldComboBox.Text & " BETWEEN '" & StartDateTimePicker.DateTime.ToString("yyy-MM-dd") & "' AND '" & EndDateTimePicker.DateTime.ToString("yyyy-MM-dd") & "' ORDER BY FileType DESC "
                Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

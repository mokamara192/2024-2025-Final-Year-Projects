...
Public Class frmBank
    ...
    Dim LoadedBankAccountID As String
    ...
    Private Sub LoadRecords()
        Try
            ...
            Dim Query As String = " SELECT  BankAccountID,NameOfBank,AccountName,AccountNo,Address,Comment,RegDate,InputedBy FROM Bank ORDER BY BankAccountID DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

            MyReportTable.Rows.Clear()
            ConTableAdapter.Fill(MyReportTable)

            BankDataGridView.DataSource = MyReportTable
            isDateSearchUsed = False
            'If MyReportTable.Rows.Count > 0 Then
            'DisplayRecordInfo(MyReportTable.Rows(0).Item("BankAccountID"))
            'End If
    ...
    Private Sub cmdAdd_Click() 'Handles cmdAdd.Click
        ...
        If Not isCalled Then
            LoadedBankAccountID = ""
        End If
        BankAccountIDTextBox.Text = ""
        NameOfBankTextBox.Text = ""
    ...
    Private Sub cmdSave_Click() 'Handles cmdSave.Click
        Try
            ...
            Dim Query As String = "INSERT INTO Bank (NameOfBank,AccountName,AccountNo,Address,Comment,RegDate,InputedBy) VALUES(@NameOfBank,@AccountName,@AccountNo,@Address,@Comment,@RegDate,@InputedBy)"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@BankAccountID", BankAccountIDTextBox.Text)
                .AddWithValue("@NameOfBank", NameOfBankTextBox.Text)
                .AddWithValue("@AccountName", AccountNameTextBox.Text)
                .AddWithValue("@AccountNo", AccountNoTextBox.Text)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "Bank" & " With ID (" & BankAccountIDTextBox.Text & ")", 0)
            LoadRecords()
            'cmdAdd_Click()
            DisplayRecordInfo(BankAccountIDTextBox.Text)
    ...
    Private Sub cmdUpdate_Click() 'Handles cmdUpdate.Click
        Try
            ...
            Dim Query As String = "UPDATE Bank SET NameOfBank =@NameOfBank, AccountName =@AccountName, AccountNo =@AccountNo, Address =@Address, Comment =@Comment, RegDate =@RegDate, InputedBy =@InputedBy WHERE BankAccountID = '" & LoadedBankAccountID & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@BankAccountID", BankAccountIDTextBox.Text)
                .AddWithValue("@NameOfBank", NameOfBankTextBox.Text)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "Bank" & " With ID (" & BankAccountIDTextBox.Text & ")", 0)
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that  have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
        ...
        If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                If PanelNew.Visible Then
                    ThePKey = BankAccountIDTextBox.Text
                Else
                    ThePKey = GridView1.GetFocusedDataRow.Item("BankAccountID")
                End If

                Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                Dim Query As String = "DELETE FROM Bank WHERE BankAccountID = '" & ThePKey & "'"
                Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                ...
                CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "Bank" & " With ID (" & ThePKey & ")", 0)
                LoadRecords()
                Dim TheNewPKey As String = ""
                If MyReportTable.Rows.Count > 0 Then
                    TheNewPKey = GridView1.GetFocusedDataRow.Item("BankAccountID")
                End If
    ...
    'Private Sub BankAccountIDTextBox_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles BankAccountIDTextBox.TextChanged
    'DisplayRecordInfo(BankAccountIDTextBox.Text)
    'End Sub
    Public Sub DisplayRecordInfo(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = " SELECT BankAccountID,NameOfBank,AccountName,AccountNo,Address,Comment,RegDate,InputedBy FROM Bank WHERE BankAccountID = '" & RecordID & "'"
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
            ...
            If MySingleReportTable.Rows.Count > 0 Then
                ...
                BankAccountIDTextBox.Text = MySingleReportTable.Rows(0).Item("BankAccountID").ToString
                NameOfBankTextBox.Text = MySingleReportTable.Rows(0).Item("NameOfBank").ToString
                AccountNameTextBox.Text = MySingleReportTable.Rows(0).Item("AccountName").ToString
                AccountNoTextBox.Text = MySingleReportTable.Rows(0).Item("AccountNo").ToString
                AddressTextBox.Text = MySingleReportTable.Rows(0).Item("Address").ToString
                ...
                LoadTransfersRecords(BankAccountIDTextBox.Text)
                LoadedBankAccountID = BankAccountIDTextBox.Text
...
#Region "Transfers"
    ...
    Private Sub LoadTransfersRecords(ByVal RecordID As String)
        Try
            Dim Conn As New SqlClient.SqlConnection(MyConnectionString)

            Dim Query As String = "SELECT GeneratedTransferNo,OutgoneTransfer,TransferNo,TransferType,TransferDate,OfficeType,TotalAmount,TransferStatus,Comment,Supervision,DocumentCompletion,RegDate,InputtedBy,Currency,ExchangeRate,BankAccountID FROM Transfers WHERE BankAccountID = '" & RecordID & "' ORDER BY GeneratedTransferNo DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
            ...
            TransfersGridView.DataSource = MYTABLE
    ...
    Private Sub cmdAddTransfers_Click(sender As Object, e As EventArgs) Handles cmdAddTransfers.Click
        Try
            Dim MyForm As New frmTransfers
            MyForm.isCalled = True
            MyForm.PrepareForm()
            MyForm.NewToolStripButton.PerformClick()
            MyForm.BankAccountIDGridBox.EditValue = Me.BankAccountIDTextBox.Text
    ...
    Private Sub cmdRemoveTransfers_Click(sender As Object, e As EventArgs) Handles cmdRemoveTransfers.Click
        ...
        If RecordAccessLevel.Contains("Delete") Then
            If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
                Try
                    '**************************
                    '**************************
                    Dim RecordID As String = GridViewTransfers.GetFocusedRowCellDisplayText(GridViewTransfers.Columns("GeneratedTransferNo"))

                    Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                    Dim Query As String = "DELETE FROM Transfers WHERE GeneratedTransferNo = '" & RecordID & "'"
                    Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                    ...
                    CreateLog(CurrentUserID, FullNameofUser, "Delete of Transfers for " + BankAccountIDTextBox.Text, 0)
                    LoadTransfersRecords(BankAccountIDTextBox.Text)
    ...
    Private Sub cmdOpenRecordTransfers_Click(sender As Object, e As EventArgs) Handles cmdOpenRecordTransfers.Click

        Try
            Dim TransfersID As String = GridViewTransfers.GetFocusedRowCellDisplayText(GridViewTransfers.Columns("GeneratedTransferNo"))
            Dim TransfersForm As New frmTransfers
            TransfersForm.FormBorderStyle = FormBorderStyle.Sizable
            TransfersForm.StartPosition = FormStartPosition.CenterParent
            TransfersForm.PrepareForm()

            TransfersForm.OpenNEWToolStripButton_Click()
            TransfersForm.DisplayRecordInfo(TransfersID)
    ...
    Private Sub cmdSearchTransfers_Click(sender As Object, e As EventArgs) Handles cmdSearchTransfers.Click
        Me.GridViewTransfers.OptionsFind.AlwaysVisible = Not Me.GridViewTransfers.OptionsFind.AlwaysVisible
...
#End Region
    ...
    Private Function ValidateAllForeignKeys() As Boolean
        ...
        'If Not ValidateUniversalID("Transfers", "GeneratedTransferNo", GeneratedTransferNoGridBox.EditValue) Then
        '    MessageBox.Show("Please provide an acceptable value for '"Generated Transfer No & "' from its drop-down list before saving the record.", "Missing Value", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        '    Return False
        'End If
    ...
    '       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportBank
    '       MyDocument.SummaryInfo.ReportTitle = "Bank"
    '	      Dim MyTable As New DataTable
    '       Dim TableName As String = "Bank"
    '       Dim Fields As String = "BankAccountID"
    '       Dim Values As String = BankAccountIDTextBox.Text
    ...
    '           Dim QrCodeData As String = "Bank Report - Printed on: " & DateTime.Today
    ...
    Private Sub SearchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SearchToolStripMenuItem.Click
        ...
        Try
            ...
            MySearch.FormName = "frmBank"
            MySearch.TableName = "Bank"
            MySearch.PrimaryKey = "BankAccountID"
    ...
    Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("Bank")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("Bank")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub OpenToolStripButton_Click() Handles OpenToolStripButton.Click
        Try
            Dim DRecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("BankAccountID"))
    ...
    Private Sub SearchToolStripButton_Click() Handles SearchToolStripButton.Click
        ...
        Try
            Dim QueryItems As String = "BankAccountID,NameOfBank,AccountName,AccountNo,Address,Comment,RegDate,InputedBy"
            Dim TableName As String = "Bank"
            ...
            If SearchToolStripComboBox.Text = "" Then
                Condition = ""
            Else
                Condition = "BankAccountID = '" & SearchToolStripComboBox.Text & "'"
            End If

            Dim OrderByCondition As String = "ORDER BY BankAccountID DESC"
            ...
            Else
                OpenNEWToolStripButton_Click()
                DisplayRecordInfo(MyTable.Rows(0).Item("BankAccountID"))
            End If
            isDateSearchUsed = False
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub MySearchItemsLoading() Handles Me.Load
        Dim QueryItem As String = "BankAccountID"
        Dim TableName As String = "Bank"
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "Bank", "BankAccountID = '" & DRecordID & "'", " ORDER BY BankAccountID ASC")
...
#Region "EXPORTING AND OTHER FUNCTIONS FROM DASHBOARD BUTTONS"
    ...
    Private Sub frmBank_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
#End Region
    ...
    Private Sub PrintToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintToolStripMenuItem.Click
        Try
            ...
            If PanelNew.Visible = False Then
                Dim RecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("BankAccountID"))
            ...
            Dim MyDocument As New XtraReportBankSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.BankDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "Bank"
    ...
    Private Sub PrintAllListedRecordsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintAllListedRecordsToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportBankMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.BankDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "Bank"
...
#Region "SEARCH BY DATE"
    ...
    Private Sub FormLoadingSetup() Handles Me.Load
        Try
            ...
            Dim Allfields As String = GetAllFields("Bank", DatabaseName)
            ...
            For Each field In splitfields
                ...
                Dim ColumnType As String = GetColumnDataType("Bank", field).ToUpper
    ...
    Private Sub cmdDateSearch_Click(sender As Object, e As EventArgs) Handles cmdDateSearch.Click
        If SearchDateFieldComboBox.SelectedIndex <> -1 Then
            Try
                ...
                Dim Query As String = " SELECT BankAccountID,NameOfBank,AccountName,AccountNo,Address,Comment,RegDate,InputedBy FROM Bank WHERE " & SearchDateFieldComboBox.Text & " BETWEEN '" & StartDateTimePicker.DateTime.ToString("yyy-MM-dd") & "' AND '" & EndDateTimePicker.DateTime.ToString("yyyy-MM-dd") & "' ORDER BY BankAccountID DESC "
                Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

...
Public Class frmBranch
    ...
    Private Sub LoadRecords()
        Try
            ...
            Dim Query As String = " SELECT  BranchID,BranchName,BranchCity,BranchAddress,BranchPhoneNo,RegDate,InputedBy FROM Branch ORDER BY BranchID DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

            MyReportTable.Rows.Clear()
            ConTableAdapter.Fill(MyReportTable)

            BranchDataGridView.DataSource = MyReportTable
            isDateSearchUsed = False
            'If MyReportTable.Rows.Count > 0 Then
            'DisplayRecordInfo(MyReportTable.Rows(0).Item("BranchID"))
            'End If
    ...
    Private Sub cmdSave_Click() 'Handles cmdSave.Click
        Try
            ...
            Dim Query As String = "INSERT INTO Branch (BranchName,BranchCity,BranchAddress,BranchPhoneNo,RegDate,InputedBy) VALUES(@BranchName,@BranchCity,@BranchAddress,@BranchPhoneNo,@RegDate,@InputedBy)"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@BranchID", BranchIDTextBox.Text)
                .AddWithValue("@BranchName", BranchNameTextBox.Text)
                .AddWithValue("@BranchCity", BranchCityTextBox.Text)
                .AddWithValue("@BranchAddress", BranchAddressTextBox.Text)
                .AddWithValue("@BranchPhoneNo", BranchPhoneNoTextBox.Text)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "Branch" & " With ID (" & BranchIDTextBox.Text & ")", 0)
            LoadRecords()
            'cmdAdd_Click()
            DisplayRecordInfo(BranchIDTextBox.Text)
    ...
    Private Sub cmdUpdate_Click() 'Handles cmdUpdate.Click
        Try
            ...
            Dim Query As String = "UPDATE Branch SET BranchName =@BranchName, BranchCity =@BranchCity, BranchAddress =@BranchAddress, BranchPhoneNo =@BranchPhoneNo, RegDate =@RegDate, InputedBy =@InputedBy WHERE BranchID = '" & LoadedBranchID & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@BranchID", BranchIDTextBox.Text)
                .AddWithValue("@BranchName", BranchNameTextBox.Text)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "Branch" & " With ID (" & BranchIDTextBox.Text & ")", 0)
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that  have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
        ...
        If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                If PanelNew.Visible Then
                    ThePKey = BranchIDTextBox.Text
                Else
                    ThePKey = GridView1.GetFocusedDataRow.Item("BranchID")
                End If

                Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                Dim Query As String = "DELETE FROM Branch WHERE BranchID = '" & ThePKey & "'"
                Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                ...
                CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "Branch" & " With ID (" & ThePKey & ")", 0)
                LoadRecords()
                Dim TheNewPKey As String = ""
                If MyReportTable.Rows.Count > 0 Then
                    TheNewPKey = GridView1.GetFocusedDataRow.Item("BranchID")
                End If
    ...
    'Private Sub BranchIDTextBox_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles BranchIDTextBox.TextChanged
    'DisplayRecordInfo(BranchIDTextBox.Text)
    'End Sub
    Public Sub DisplayRecordInfo(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = " SELECT BranchID,BranchName,BranchCity,BranchAddress,BranchPhoneNo,RegDate,InputedBy FROM Branch WHERE BranchID = '" & RecordID & "'"
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
            ...
            If MySingleReportTable.Rows.Count > 0 Then
                ...
                BranchIDTextBox.Text = MySingleReportTable.Rows(0).Item("BranchID").ToString
                BranchNameTextBox.Text = MySingleReportTable.Rows(0).Item("BranchName").ToString
                BranchCityTextBox.Text = MySingleReportTable.Rows(0).Item("BranchCity").ToString
                BranchAddressTextBox.Text = MySingleReportTable.Rows(0).Item("BranchAddress").ToString
                BranchPhoneNoTextBox.Text = MySingleReportTable.Rows(0).Item("BranchPhoneNo").ToString
                ...
                LoadedBranchID = BranchIDTextBox.Text
    ...
    '       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportBranch
    '       MyDocument.SummaryInfo.ReportTitle = "Branch"
    '	      Dim MyTable As New DataTable
    '       Dim TableName As String = "Branch"
    '       Dim Fields As String = "BranchID"
    '       Dim Values As String = BranchIDTextBox.Text
    ...
    '           Dim QrCodeData As String = "Branch Report - Printed on: " & DateTime.Today
    ...
    Private Sub SearchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SearchToolStripMenuItem.Click
        ...
        Try
            ...
            MySearch.FormName = "frmBranch"
            MySearch.TableName = "Branch"
            MySearch.PrimaryKey = "BranchID"
    ...
    Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("Branch")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("Branch")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub OpenToolStripButton_Click() Handles OpenToolStripButton.Click
        Try
            Dim DRecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("BranchID"))
    ...
    Private Sub SearchToolStripButton_Click() Handles SearchToolStripButton.Click
        ...
        Try
            Dim QueryItems As String = "BranchID,BranchName,BranchCity,BranchAddress,BranchPhoneNo,RegDate,InputedBy"
            Dim TableName As String = "Branch"
            ...
            If SearchToolStripComboBox.Text = "" Then
                Condition = ""
            Else
                Condition = "BranchID = '" & SearchToolStripComboBox.Text & "'"
            End If

            Dim OrderByCondition As String = "ORDER BY BranchID DESC"
            ...
            Else
                OpenNEWToolStripButton_Click()
                DisplayRecordInfo(MyTable.Rows(0).Item("BranchID"))
            End If
            isDateSearchUsed = False
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub MySearchItemsLoading() Handles Me.Load
        Dim QueryItem As String = "BranchID"
        Dim TableName As String = "Branch"
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "Branch", "BranchID = '" & DRecordID & "'", " ORDER BY BranchID ASC")
...
#Region "EXPORTING AND OTHER FUNCTIONS FROM DASHBOARD BUTTONS"
    ...
    Private Sub frmBranch_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
#End Region
    ...
    Private Sub PrintToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintToolStripMenuItem.Click
        Try
            ...
            If PanelNew.Visible = False Then
                Dim RecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("BranchID"))
            ...
            Dim MyDocument As New XtraReportBranchSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.BranchDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "Branch"
    ...
    Private Sub PrintAllListedRecordsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintAllListedRecordsToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportBranchMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.BranchDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "Branch"
...
#Region "SEARCH BY DATE"
    ...
    Private Sub FormLoadingSetup() Handles Me.Load
        Try
            ...
            Dim Allfields As String = GetAllFields("Branch", DatabaseName)
            ...
            For Each field In splitfields
                ...
                Dim ColumnType As String = GetColumnDataType("Branch", field).ToUpper
    ...
    Private Sub cmdDateSearch_Click(sender As Object, e As EventArgs) Handles cmdDateSearch.Click
        If SearchDateFieldComboBox.SelectedIndex <> -1 Then
            Try
                ...
                Dim Query As String = " SELECT BranchID,BranchName,BranchCity,BranchAddress,BranchPhoneNo,RegDate,InputedBy FROM Branch WHERE " & SearchDateFieldComboBox.Text & " BETWEEN '" & StartDateTimePicker.DateTime.ToString("yyy-MM-dd") & "' AND '" & EndDateTimePicker.DateTime.ToString("yyyy-MM-dd") & "' ORDER BY BranchID DESC "
                Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

...
Public Class frmTransferOffices
    ...
    Private Sub LoadRecords()
        Try
            ...
            Dim Query As String = " SELECT  OfficeID,OfficeType,NameOfOffice,ResidentialAddress,PhoneNo,EmailAddress,Occupation,OfficeOfWork,WorkAddress,IdentificationType,IdentificationNo,Photograph,Comment,RegDate,InputedBy FROM TransferOffices ORDER BY OfficeID DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

            MyReportTable.Rows.Clear()
            ConTableAdapter.Fill(MyReportTable)

            TransferOfficesDataGridView.DataSource = MyReportTable
            isDateSearchUsed = False
            'If MyReportTable.Rows.Count > 0 Then
            'DisplayRecordInfo(MyReportTable.Rows(0).Item("OfficeID"))
            'End If
    ...
    Private Function ValidateID(ByVal OfficeID As String) As Boolean
        Try
            Dim Conn As New SqlClient.SqlConnection(MyConnectionString)

            Dim Query As String = "SELECT Count(*) FROM TransferOffices WHERE OfficeID = '" & OfficeID & "'"
    ...
    Private Function GenerateID() As String
        Try
            ...
            Dim Query As String = "SELECT Count(*) FROM TransferOffices WHERE OfficeID LIKE 'CUS" & DateTime.Today.ToString("yyMMdd") & "%'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            ...
            While IsIDGenerated = False
                ...
                If Recordcount >= 0 And Recordcount < 10 Then
                    Start = "CUS" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "000" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 10 And Recordcount < 100 Then
                    Start = "CUS" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "00" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 100 And Recordcount < 1000 Then
                    Start = "CUS" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "0" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
                ElseIf Recordcount >= 1000 And Recordcount < 10000 Then
                    Start = "CUS" & CurrentUserID & DateTime.Today.ToString("yyMMdd") & "" & Recordcount
                    If ValidateID(Start) = False Then
                        Return Start
                        Exit While
                    End If
    ...
    Private Sub cmdAdd_Click() 'Handles cmdAdd.Click
        ...
        NameOfOfficeTextBox.Text = ""
        ResidentialAddressTextBox.Text = ""
        PhoneNoTextBox.Text = ""
        EmailAddressTextBox.Text = ""
        OccupationTextBox.Text = ""
        OfficeOfWorkTextBox.Text = ""
        WorkAddressTextBox.Text = ""
        IdentificationTypeTextBox.Text = ""
        ...
        OfficeIDTextBox.Text = GenerateID()

    End Sub

    Private Sub cmdSave_Click() 'Handles cmdSave.Click
        Try
            If OfficeIDTextBox.Text <> "" Then
                If ValidateID(OfficeIDTextBox.Text) Then
                    MessageBox.Show("This Office ID already exist in this table. DUPLICATE 'Office ID' ARE NOT ALLOWED. Please verify and try again.", "Already Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "INSERT INTO TransferOffices (OfficeID,OfficeType,NameOfOffice,ResidentialAddress,PhoneNo,EmailAddress,Occupation,OfficeOfWork,WorkAddress,IdentificationType,IdentificationNo,Photograph,Comment,RegDate,InputedBy) VALUES(@OfficeID,@OfficeType,@NameOfOffice,@ResidentialAddress,@PhoneNo,@EmailAddress,@Occupation,@OfficeOfWork,@WorkAddress,@IdentificationType,@IdentificationNo,@Photograph,@Comment,@RegDate,@InputedBy)"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@OfficeID", OfficeIDTextBox.Text)
                .AddWithValue("@OfficeType", OfficeTypeTextBox.Text)
                .AddWithValue("@NameOfOffice", NameOfOfficeTextBox.Text)
                .AddWithValue("@ResidentialAddress", ResidentialAddressTextBox.Text)
                .AddWithValue("@PhoneNo", PhoneNoTextBox.Text)
                .AddWithValue("@EmailAddress", EmailAddressTextBox.Text)
                .AddWithValue("@Occupation", OccupationTextBox.Text)
                .AddWithValue("@OfficeOfWork", OfficeOfWorkTextBox.Text)
                .AddWithValue("@WorkAddress", WorkAddressTextBox.Text)
                .AddWithValue("@IdentificationType", IdentificationTypeTextBox.Text)
                .AddWithValue("@IdentificationNo", IdentificationNoTextBox.Text)
                If PhotographPictureBox.Image IsNot Nothing Then
                    Dim MyMemoryStream As New System.IO.MemoryStream
                    PhotographPictureBox.Image.Save(MyMemoryStream, System.Drawing.Imaging.ImageFormat.Jpeg)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Insert of new record in table (" & "TransferOffices" & " With ID (" & OfficeIDTextBox.Text & ")", 0)
            LoadRecords()
            'cmdAdd_Click()
            DisplayRecordInfo(OfficeIDTextBox.Text)
    ...
    Private Sub cmdUpdate_Click() 'Handles cmdUpdate.Click
        Try
            If Not ValidateID(LoadedOfficeID) Then
                MessageBox.Show("The loaded Office ID (" & LoadedOfficeID & ") was not found in the database table. UPDATE OPERATION WILL NOT BE SUCCESSFULL. Please verify and try again.", "Not Exist", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            Dim Query As String = "UPDATE TransferOffices SET OfficeType =@OfficeType, NameOfOffice =@NameOfOffice, ResidentialAddress =@ResidentialAddress, PhoneNo =@PhoneNo, EmailAddress =@EmailAddress, Occupation =@Occupation, OfficeOfWork =@OfficeOfWork, WorkAddress =@WorkAddress, IdentificationType =@IdentificationType, IdentificationNo =@IdentificationNo, Photograph =@Photograph, Comment =@Comment, RegDate =@RegDate, InputedBy =@InputedBy WHERE OfficeID = '" & LoadedOfficeID & "'"
            Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
            With ConCommand.Parameters
                .AddWithValue("@OfficeID", OfficeIDTextBox.Text)
                .AddWithValue("@OfficeType", OfficeTypeTextBox.Text)
            ...
            CreateLog(CurrentUserID, FullNameofUser, "Update of record in table (" & "TransferOffices" & " With ID (" & OfficeIDTextBox.Text & ")", 0)
        Catch Ex As Exception
            If Ex.Message.ToUpper.Contains("FOREIGN") Then
                MessageBox.Show("Record Constraint Violation. Please make sure that  have values. If any does not have an applicable value then please select 'N/A' from the drop down list." & Ex.Message, "Incomplete entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Else
        ...
        If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            Try
                ...
                If PanelNew.Visible Then
                    ThePKey = OfficeIDTextBox.Text
                Else
                    ThePKey = GridView1.GetFocusedDataRow.Item("OfficeID")
                End If

                Dim Conn As New SqlClient.SqlConnection(MyConnectionString)
                Dim Query As String = "DELETE FROM TransferOffices WHERE OfficeID = '" & ThePKey & "'"
                Dim ConCommand As New SqlClient.SqlCommand(Query, Conn)
                ...
                CreateLog(CurrentUserID, FullNameofUser, "Delete of record in table (" & "TransferOffices" & " With ID (" & ThePKey & ")", 0)
                LoadRecords()
                Dim TheNewPKey As String = ""
                If MyReportTable.Rows.Count > 0 Then
                    TheNewPKey = GridView1.GetFocusedDataRow.Item("OfficeID")
                End If
    ...
    'Private Sub OfficeIDTextBox_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles OfficeIDTextBox.TextChanged
    'DisplayRecordInfo(OfficeIDTextBox.Text)
    'End Sub
    Public Sub DisplayRecordInfo(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = " SELECT OfficeID,OfficeType,NameOfOffice,ResidentialAddress,PhoneNo,EmailAddress,Occupation,OfficeOfWork,WorkAddress,IdentificationType,IdentificationNo,Photograph,Comment,RegDate,InputedBy FROM TransferOffices WHERE OfficeID = '" & RecordID & "'"
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
            ...
            If MySingleReportTable.Rows.Count > 0 Then
                ...
                OfficeIDTextBox.Text = MySingleReportTable.Rows(0).Item("OfficeID").ToString
                OfficeTypeTextBox.Text = MySingleReportTable.Rows(0).Item("OfficeType").ToString
                NameOfOfficeTextBox.Text = MySingleReportTable.Rows(0).Item("NameOfOffice").ToString
                ResidentialAddressTextBox.Text = MySingleReportTable.Rows(0).Item("ResidentialAddress").ToString
                PhoneNoTextBox.Text = MySingleReportTable.Rows(0).Item("PhoneNo").ToString
                EmailAddressTextBox.Text = MySingleReportTable.Rows(0).Item("EmailAddress").ToString
                OccupationTextBox.Text = MySingleReportTable.Rows(0).Item("Occupation").ToString
                OfficeOfWorkTextBox.Text = MySingleReportTable.Rows(0).Item("OfficeOfWork").ToString
                WorkAddressTextBox.Text = MySingleReportTable.Rows(0).Item("WorkAddress").ToString
                IdentificationTypeTextBox.Text = MySingleReportTable.Rows(0).Item("IdentificationType").ToString
                IdentificationNoTextBox.Text = MySingleReportTable.Rows(0).Item("IdentificationNo").ToString
                Try
                    Dim MyByte As Byte() = MySingleReportTable.Rows(0).Item("Photograph")

                    Dim MyStream As System.IO.MemoryStream = New System.IO.MemoryStream(MyByte)
                    PhotographPictureBox.Image = Image.FromStream(MyStream)
                Catch ex As Exception
                    PhotographPictureBox.Image = Nothing
                End Try
                ...
                LoadTransferOfficeLinkRecords(OfficeIDTextBox.Text)
                LoadedOfficeID = OfficeIDTextBox.Text
...
#Region "TransferOfficeLink"
    ...
    Private Sub LoadTransferOfficeLinkRecords(ByVal RecordID As String)
        Try
            ...
            Dim Query As String = "SELECT TransferOfficeID,GeneratedTransferNo,OfficeID,RegDate,InputedBy FROM TransferOfficeLink WHERE OfficeID = '" & RecordID & "' ORDER BY TransferOfficeID DESC "
            Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)
    ...
    Private Sub cmdAddTransferOfficeLink_Click(sender As Object, e As EventArgs) Handles cmdAddTransferOfficeLink.Click
        Try
            ...
            MyForm.OfficeIDGridBox.EditValue = Me.OfficeIDTextBox.Text
        ...
        If RecordAccessLevel.Contains("Delete") Then
            If MessageBox.Show("Do you really want to delete the current record?", "Caution", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
                Try
                    ...
                    CreateLog(CurrentUserID, FullNameofUser, "Delete of TransferOfficeLink for " + OfficeIDTextBox.Text, 0)
                    LoadTransferOfficeLinkRecords(OfficeIDTextBox.Text)
...
#End Region
    ...
    '       Dim MyDocument As CrystalDecisions.CrystalReports.Engine.ReportDocument '= New CrystalReportTransferOffices
    '       MyDocument.SummaryInfo.ReportTitle = "Transfer Offices"
    '	      Dim MyTable As New DataTable
    '       Dim TableName As String = "TransferOffices"
    '       Dim Fields As String = "OfficeID"
    '       Dim Values As String = OfficeIDTextBox.Text
    ...
    '           Dim QrCodeData As String = "TransferOffices Report - Printed on: " & DateTime.Today
    ...
    Private Sub cmdBrowsePhotograph_Click() Handles cmdBrowsePhotograph.Click
        ...
        Try
            ...
            If OFD1.ShowDialog = System.Windows.Forms.DialogResult.OK Then
                PhotographPictureBox.Image = Image.FromFile(OFD1.FileName)
            End If
    ...
    Private Sub cmdCameraPhotograph_Click(sender As Object, e As EventArgs) Handles cmdCameraPhotograph.Click
        Try
    ...
    Private Sub SearchToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SearchToolStripMenuItem.Click
        ...
        Try
            ...
            MySearch.FormName = "frmTransferOffices"
            MySearch.TableName = "TransferOffices"
            MySearch.PrimaryKey = "OfficeID"
    ...
    Private Sub AdvancedSummaryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdvancedSummaryToolStripMenuItem.Click
        Try
            ...
            myReport.LoadTables()
            myReport.Show(Me)
            myReport.cboTableName.SelectedIndex = myReport.FindItemIndex("TransferOffices")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub ImportWizardToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportWizardToolStripMenuItem.Click
        Try
            ...
            MyImportForm.cboTableName.SelectedIndex = MyImportForm.FindItemIndex("TransferOffices")
...
#Region "TOOLSTRIP SECTION"
    ...
    Private Sub OpenToolStripButton_Click() Handles OpenToolStripButton.Click
        Try
            Dim DRecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("OfficeID"))
    ...
    Private Sub SearchToolStripButton_Click() Handles SearchToolStripButton.Click
        ...
        Try
            Dim QueryItems As String = "OfficeID,OfficeType,NameOfOffice,ResidentialAddress,PhoneNo,EmailAddress,Occupation,OfficeOfWork,WorkAddress,IdentificationType,IdentificationNo,Photograph,Comment,RegDate,InputedBy"
            Dim TableName As String = "TransferOffices"
            ...
            If SearchToolStripComboBox.Text = "" Then
                Condition = ""
            Else
                Condition = "OfficeID = '" & SearchToolStripComboBox.Text & "'"
            End If

            Dim OrderByCondition As String = "ORDER BY OfficeID DESC"
            ...
            Else
                OpenNEWToolStripButton_Click()
                DisplayRecordInfo(MyTable.Rows(0).Item("OfficeID"))
            End If
            isDateSearchUsed = False
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error Occured!", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub MySearchItemsLoading() Handles Me.Load
        Dim QueryItem As String = "OfficeID"
        Dim TableName As String = "TransferOffices"
    ...
    Private Sub DataGridView_FocusedRowChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim MyTable As DataTable = GetDataForTable(" * ", "TransferOffices", "OfficeID = '" & DRecordID & "'", " ORDER BY OfficeID ASC")
...
#Region "EXPORTING AND OTHER FUNCTIONS FROM DASHBOARD BUTTONS"
    ...
    Private Sub frmTransferOffices_GotFocus(sender As Object, e As EventArgs) Handles Me.Activated
...
#End Region
    ...
    Private Sub PrintToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintToolStripMenuItem.Click
        Try
            ...
            If PanelNew.Visible = False Then
                Dim RecordID As String = GridView1.GetFocusedRowCellDisplayText(GridView1.Columns("OfficeID"))
            ...
            Dim MyDocument As New XtraReportTransferOfficesSingle
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.TransferOfficesDataSource.DataSource = MySingleReportTable
            MyDocument.XrLabelReportTitle.Text = "Transfer Offices"
    ...
    Private Sub PrintAllListedRecordsToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PrintAllListedRecordsToolStripMenuItem.Click
        Try
            ...
            Dim MyDocument As New XtraReportTransferOfficesMultiple
            MyDocument.CompanySettingsDataSource.DataSource = MyCompanyTable
            MyDocument.TransferOfficesDataSource.DataSource = MyReportTable
            MyDocument.XrLabelReportTitle.Text = "Transfer Offices List"
...
#Region "SEARCH BY DATE"
    ...
    Private Sub FormLoadingSetup() Handles Me.Load
        Try
            ...
            Dim Allfields As String = GetAllFields("TransferOffices", DatabaseName)
            ...
            For Each field In splitfields
                ...
                Dim ColumnType As String = GetColumnDataType("TransferOffices", field).ToUpper
    ...
    Private Sub cmdDateSearch_Click(sender As Object, e As EventArgs) Handles cmdDateSearch.Click
        If SearchDateFieldComboBox.SelectedIndex <> -1 Then
            Try
                ...
                Dim Query As String = " SELECT OfficeID,OfficeType,NameOfOffice,ResidentialAddress,PhoneNo,EmailAddress,Occupation,OfficeOfWork,WorkAddress,IdentificationType,IdentificationNo,Photograph,Comment,RegDate,InputedBy FROM TransferOffices WHERE " & SearchDateFieldComboBox.Text & " BETWEEN '" & StartDateTimePicker.DateTime.ToString("yyy-MM-dd") & "' AND '" & EndDateTimePicker.DateTime.ToString("yyyy-MM-dd") & "' ORDER BY OfficeID DESC "
                Dim ConTableAdapter As New SqlClient.SqlDataAdapter(Query, Conn)

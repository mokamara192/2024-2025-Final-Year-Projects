...
from homepage.models import donatemoney,donatevaluables,Orphanage,Transport
...
def donatemoneyview(request):
    ...
    if request.method == 'POST':
        ...
        status = "0"
        saveform = donatemoney.objects.create(
            user_id=user_id, transfer=transfer, amount=amount, orphanage_id=orphanage, orphanage_name=orphanage_name,
...
def donatevaluablesview(request):
    ...
    if request.method == 'POST':
        ...
        # Save the form data
        saveform = donatevaluables.objects.create(
            user_id=user_id,
            donation_type=type,
        ...
        messages.success(request, 'Donation of valuables has been successfully placed!')

        # Refresh the same page (no redirection)
        return render(request, 'donation/valuables.html', {'orphanages': Orphanages})

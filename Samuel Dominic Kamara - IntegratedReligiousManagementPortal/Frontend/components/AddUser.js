...
const AddUser = () => {
    ...
    const saveUser = async (e) => {
        ...
        try {
            const checkResponse = await fetch(`${USER_API_BASE_URL}/check-email?email=${user.emailId}`);
            if (!checkResponse.ok) {
        ...
        // Save user if email does not exist
        try {
    ...
    const reset = (e) => {
        ...
        setUser({
            ...
            role: "Affiliate", // Reset to default role
        });
        setError("");
        setIsOpen(false);
    };
        ...
        <>
            <div className="container mx-auto my-8">
                <div className="h-12">
                    <button
                        onClick={openModal}
                        className="rounded bg-slate-600 text-white px-4 py-2 font-semibold"
                    >
